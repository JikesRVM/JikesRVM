...
class Thread implements Runnable {
  ...
  /*We may not need this registerNative in JikesRVM*/
  ...
  private static ThreadGroup initThreadGroup = null;
  ...
  public static void sleep(long millis) throws InterruptedException{
     ...
     */
  private void init(RVMThread vmt, ThreadGroup g, Runnable target, String name,
                      long stackSize) {
	...
	if (parent == null){
	  /*We need build a wrap for RVMThread*/
	  if ((g == null) && (systemThreadGroup == null)){
	    systemThreadGroup = new ThreadGroup();
	    initThreadGroup = new ThreadGroup(systemThreadGroup,"main");
	  }
	  
	  if (g==null)
	    this.group = initThreadGroup;
	  else
	    this.group = g;

	  this.daemon = vmThread.isDaemonThread();
	  this.priority = vmThread.getPriority();
	...
	//First normal thread should be here
	...
	if (g == null) 
	  VM.sysWriteln("Thread.java: g is null");
        g.checkAccess();
  ...
  /*Added constructor for RVM interface*/
  Thread(RVMThread vmt, String name){
    init(vmt,null,null,name,0);
  }
     ...
     */
    public Thread() {
      init(null,null, null, "Thread-" + nextThreadNum(), 0);
    }
    ...
    public Thread(Runnable target) {
      init(null,null, target, "Thread-" + nextThreadNum(), 0);
    }
    ...
    public Thread(ThreadGroup group, Runnable target) {
      init(null,group, target, "Thread-" + nextThreadNum(), 0);
    }
    ...
    public Thread(ThreadGroup group, Runnable target, String name,
                  long stackSize) {
      init(null, group, target, name, stackSize);
    }
