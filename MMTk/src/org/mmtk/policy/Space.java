...
/**
 * This class defines and manages spaces. Each policy is an instance of a space.
 * A space is a region of virtual memory (contigious or discontigous) which is
 * subject to the same memory management regime. Multiple spaces (instances of
 * this class or its descendants) may have the same policy (eg there could be
 * numerous instances of CopySpace, each with different roles). Spaces are
 * defined in terms of a unique region of virtual memory, so no two space
 * instances ever share any virtual memory.
 * <p>
 * 
 * In addition to tracking virtual memory use and the mapping to policy, spaces
 * also manage memory consumption (<i>used</i> virtual memory).
 * <p>
 * 
...
public abstract class Space implements Constants, Uninterruptible {
  ...
  private static final int LOG_ADDRESS_SPACE = (BYTES_IN_ADDRESS == 4) ? 32
      : 40;

  private static Address HEAP_START = chunkAlign(Memory.HEAP_START(), true);

  private static Address AVAILABLE_START = chunkAlign(Memory.AVAILABLE_START(),
      false);

  private static Address AVAILABLE_END = chunkAlign(Memory.AVAILABLE_END(),
      true);

  private static Extent AVAILABLE_BYTES = AVAILABLE_END.toWord().minus(
      AVAILABLE_START.toWord()).toExtent();
  ...
  private static final int LOG_MAX_CHUNKS = LOG_ADDRESS_SPACE
      - LOG_BYTES_IN_CHUNK;
  ...
  /**
   * This is the base constructor for <i>contigious</i> spaces (i.e. those that
   * occupy a single contigious range of virtual memory which is identified at
   * construction time).
   * <p>
   * 
   ...
   * @param movable
   *          Are objects in this space movable?
   * @param immortal
   *          Are objects in this space immortal (uncollected)?
   * @param start
  ...
  Space(String name, boolean movable, boolean immortal, Address start,
    ...
    if (bytes.NE(chunkAlign(bytes, false))) {
      ...
      Log.write(bytes.toLong());
      Log.write(" bytes rounded up to ");
  ...
  Space(String name, boolean movable, boolean immortal, int mb) {
    this(name, movable, immortal, heapCursor, Word.fromIntSignExtend(mb).lsh(
        LOG_BYTES_IN_MBYTE).toExtent());
  ...
  Space(String name, boolean movable, boolean immortal, int mb, boolean top) {
    this(name, movable, immortal, Word.fromIntSignExtend(mb).lsh(
        LOG_BYTES_IN_MBYTE).toExtent(), top);
  }
   ...
   *          virtual memory.
   */
  Space(String name, boolean movable, boolean immortal, float frac, boolean top) {
    this(name, movable, immortal, getFracAvailable(frac), top);
  }

  /**
   * This is a private constructor that creates a contigious space at the top or
   * bottom of the available virtual memory, if possible.
   * <p>
   * 
   * The caller specifies the size of the region of virtual memory and whether
   * it should be at the top or bottom of the available address space. If this
   * region conflicts with an existing space, then the constructor will fail.
   * 
   * @param name
   *          The name of this space (used when printing error messages etc)
   * @param movable
   ...
   * Immortal getter
   * 
   * @return True if this space is never collected
   */
   ...
   * Movable getter
   * 
   * @return True if objects in this space may move
   */
  ...
  /**
   * Cumulative committed pages getter
   * 
  ...
  /**
   * Return true if the given object is the space associated with the given
   * descriptor.
   * 
   * @param descriptor
   *          The descriptor for a space
   * @param object
   *          The object in question
   * @return True if the given object is in the space associated with the
   *         descriptor.
   */
  public static boolean isInSpace(int descriptor, ObjectReference object)
    ...
    if (!SpaceDescriptor.isContiguous(descriptor)) {
      ...
      if (!Assert.VERIFY_ASSERTIONS
        ...
        Extent size = Word.fromIntSignExtend(
            SpaceDescriptor.getChunks(descriptor)).lsh(LOG_BYTES_IN_CHUNK)
            .toExtent();
  ...
  /**
   * Acquire a number of pages from the page resource, returning either the
   * address of the first page, or zero on failure.
   * <p>
   * 
   ...
   * necessary. If so, the GC is polled. If a GC is required then the request
   * fails and zero is returned.
   * <p>
   * 
   * If the check of the page budget does not lead to GC being triggered, then a
   * request is made for specific pages in virtual memory. If the page manager
   * cannot satisify this request, then the request fails, a GC is forced, and
   * zero is returned. Otherwise the address of the first page is returned.
   * <p>
   * 
   * @param pages
   ...
   * 
   * @param start
   *          The address of the start of the region to be released
   */
   ...
   * 
   * @param mode
   *          An enumeration type that specifies the format for the prining
   *          (PAGES, MB, PAGES_MB, or MB_PAGES).
   */
  ...
  private static void printPages(int pages, int mode) {
    ...
    case MB_PAGES:
      ...
      break;
    default:
      Assert.fail("writePages passed illegal printing mode");
    }
  }
  ...
  /**
   * Trace an object as part of a collection and return the object, which may
   * have been forwarded (if a copying collector).
   * 
   * @param trace
   ...
   * @param down
   *          If true the address will be rounded down, otherwise it will
   *          rounded up.
   */
  private static final Address chunkAlign(Address addr, boolean down) {
    if (!down)
      addr = addr.plus(BYTES_IN_CHUNK - 1);
    return addr.toWord().rshl(LOG_BYTES_IN_CHUNK).lsh(LOG_BYTES_IN_CHUNK)
        .toAddress();
  }
  ...
  private static final Extent chunkAlign(Extent bytes, boolean down) {
    if (!down)
      bytes = bytes.plus(BYTES_IN_CHUNK - 1);
    return bytes.toWord().rshl(LOG_BYTES_IN_CHUNK).lsh(LOG_BYTES_IN_CHUNK)
        .toExtent();
  }
   ...
   * 
   * @param shared
   *          True if this is a shared (discontigious) space
   */
  ...
  /**
   * Convert a fraction into a number of bytes according to the fraction of
   * available bytes.
   * 
   * @param frac
   *          The fraction of avialable virtual memory desired
