 ...
 * 
 * Each of the instance methods of this class may be called by any thread (i.e.
 * synchronization must be explicit in any instance or class method).
 * 
 * This stands in contrast to TreadmillLocal, which is instantiated and called
 * on a per-thread basis, where each instance of TreadmillLocal corresponds to
 * one thread operating over one space.
 * 
 * $Id$
 * 
 ...
 * @date $Date$
 */
public final class LargeObjectSpace extends Space implements Constants,
    Uninterruptible {
   ...
   *          virtual memory.
   */
  public LargeObjectSpace(String name, int pageBudget, float frac, boolean top) {
    super(name, false, false, frac, top);
    pr = new FreeListPageResource(pageBudget, this, start, extent);
  }

  /**
   * Return the initial value for the header of a new object instance. The
   * header for this collector includes a mark bit and a small object flag.
   * 
   * @param size
   *          The size of the newly allocated object
   */
  public final Word getInitialHeaderValue(int size) throws InlinePragma {
    return markState;
  }
  ...
  /**
   * Prepare for a new collection increment. For the mark-sweep collector we
   * must flip the state of the mark bit between collections.
   * 
   */
  public void prepare() {
  ...
  /**
   * A new collection increment has completed. For the mark-sweep collector this
   * means we can perform the sweep phase.
   * 
   */
  public void release() {
   ...
   * @return True if this mark-sweep space is currently being collected.
   */
  public boolean inTreadmillCollection() throws InlinePragma {
  ...
  /**
   * Trace a reference to an object under a mark sweep collection policy. If the
   * object header is not already marked, mark the object in either the bitmap
   * or by moving it off the treadmill, and enqueue the object for subsequent
   * processing. The object is marked as (an atomic) side-effect of checking
   * whether already marked.
   * 
   * @param trace
   *          The trace being conducted.
   * @param object
   *          The object to be traced.
   * @return The object (there is no object forwarding in this collector, so we
   *         always return the same object: this could be a void method but for
   *         compliance to a more general interface).
   */
  ...
  /**
   * An object has been marked (identified as live). Large objects are added to
   * the to-space treadmill, while all other objects will have a mark bit set in
   * the superpage header.
   * 
   * @param object
   *          The object which has been marked.
   */
   ...
   * 
   * @param object
   *          The object whose mark bit is to be written
   * @param value
   *          The value to which the mark bit will be set
   */
  ...
  static public boolean testMarkBit(ObjectReference object, Word value)
      throws InlinePragma {
    return ObjectModel.readAvailableBitsWord(object).and(MARK_BIT_MASK).EQ(
        value);
  }
