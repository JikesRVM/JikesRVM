...
/**
 * This class implements unsynchronized (local) elements of a mark-sweep
 * collector. Allocation is via the segregated free list (@see
 * SegregatedFreeList). Marking is done using both a bit in each header's object
 * word, and a mark bitmap. Sweeping is performed lazily.
 * <p>
 * 
 * A free list block is a contigious region of memory containing cells of a
 * single size class, and is a construct of the SegregatedFreeList. This class
 * extends the block to include a mark bitmap. During the mark phase, if an
 * object is encountered with the mark bit in its header unset, it is set and
 * the mark bit in the block header corresponding to that object is set. The
 * rationale behind this approach is that testing (and setting) the mark bit in
 * the object header is cheap, while using a bitmap makes sweeping more
 * efficient. This approach maximizes the speed of the common case when marking,
 * while also allowing for fast sweeping, with minimal space overhead (2 bits
 * per object).
 * 
 ...
 * @date $Date$
 */
public final class MarkSweepLocal extends SegregatedFreeList implements
  ...
  /**
   * Class initializer. The primary task performed here is to establish the
   * block layout for each size class.
   */
  static {
   ...
   * 
   * @param space
   *          The mark-sweep space to which this allocator instances is bound.
   */
  ...
  /**
   * Print out fragmentation and wastage statistics. This is not intended to be
   * efficient, just accurate and informative.
   */
  ...
  private final void shortFragmentationStatistics(boolean prepare) {
    ...
    Log.write(waste / (float) (1 << 20));
    Log.write(" MB)");
    if (Options.verbose.getValue() > 2
        || Options.verboseFragmentationStats.getValue())
      Log.writeln();
  }
   ...
   * 
   * @param prepare
   *          True if this is called in the prepare phase (immediately prior to
   *          GC), false if called in the release phase.
   * @return The number of unused bytes on the free lists
   */
  private final int unusedBytes(boolean prepare) {
    int unused = 0;
    for (int sizeClass = 1; sizeClass < SIZE_CLASSES; sizeClass++) {
      Address block = (prepare) ? currentBlock.get(sizeClass) : firstBlock
          .get(sizeClass);
      while (!block.isZero()) {
        unused += cellSize[sizeClass]
            * (cellsInBlock[sizeClass] - markedCells(block, sizeClass));
  ...
  /**
   * Return the number of cells marked as live. The utility of this method is a
   * function of when it is called (i.e. mark bits are zeroed, set, and become
   * stale, and depending on where in this cycle this method is called, the
   * results will differ dramatically).
   * 
   * @param block
   *          The block whose marked cells are to be counted
    ...
    for (int sizeClass = 1; sizeClass < SIZE_CLASSES; sizeClass++) {
      ...
      while (!block.isZero()) {
        ...
        if (getUsed) {
          ...
          int pctl = (FRAG_PERCENTILES * marked)
              / (cellsInBlock[sizeClass] + 1);
          utilization[pctl]++;
          usedCells += marked;
          if (prepare) {
      ...
      totBytes += bytes;
      printFragRow(prepare, false, false, sizeClass, usedCellBytes, cellBytes
          - usedCellBytes, cellBytes, bytes, blocks);
    }
    printFragRow(prepare, false, true, 0, totUsedCellBytes, totCellBytes
        - totUsedCellBytes, totCellBytes, totBytes, totBlocks);
  }
  ...
  private final void finalVerboseFragmentationStatistics(boolean prepare) {
    ...
    for (int sizeClass = 1; sizeClass < SIZE_CLASSES; sizeClass++) {
      int blocks = (prepare) ? allPreBlocks[sizeClass]
          : allPostBlocks[sizeClass];
      int usedCells = (prepare) ? allPreUsedCells[sizeClass]
          : allPostUsedCells[sizeClass];
      totBlocks += blocks;
      int usedCellBytes = usedCells * cellSize[sizeClass];
      totUsedCellBytes += usedCellBytes;
      int cellBytes = (blocks * cellsInBlock[sizeClass]) * cellSize[sizeClass];
      ...
      int bytes = blocks * BlockAllocator.blockSize(blockSizeClass[sizeClass]);
      totBytes += bytes;
      printFragRow(prepare, true, false, sizeClass, usedCellBytes, cellBytes
          - usedCellBytes, cellBytes, bytes, blocks);
    }
    printFragRow(prepare, true, true, 0, totUsedCellBytes, totCellBytes
        - totUsedCellBytes, totCellBytes, totBytes, totBlocks);
  }
  ...
  private final void printFragRow(boolean prepare, boolean all, boolean totals,
      int sizeClass, int usedCellBytes, int freeBytes, int cellBytes,
      int totBytes, int blocks) {
    if (all)
      Log.write("=");
    Log.write((prepare) ? "> " : "< ");
    if (totals)
  ...
  private static void dumpSizeClassData() {
    ...
    for (int sc = 0; sc < SIZE_CLASSES; sc++) {
      Log.write(sc);
      Log.write("\t");
      Log.write(cellSize[sc]);
      Log.write("\t");
      Log.write(cellsInBlock[sc]);
      Log.write("\t");
      Log.write(blockSizeClass[sc]);
      Log.write("\t");
      Log.write(blockHeaderSize[sc]);
      Log.write("\t");
      Log.write(cellSize[sc] * cellsInBlock[sc]);
      Log.write("\t");
      // Log.write(cellSize[sc]*cellsInBlock[sc]+blockHeaderSize[sc]+Block.BLOCK_HEADER_SIZE);
      // Log.write("\t");
      Log.write(BlockAllocator.blockSize(blockSizeClass[sc])
          - (cellSize[sc] * cellsInBlock[sc] + blockHeaderSize[sc]));
      Log.write("\t");
      Log.write(((float) (cellSize[sc] * cellsInBlock[sc]))
          / ((float) BlockAllocator.blockSize(blockSizeClass[sc])));
      Log.writeln();
    }
  }
