 ...
 * @date $Date$
 */
public final class MarkCompactSpace extends Space implements Constants,
    Uninterruptible {
  ...
  private static final Offset FORWARDING_POINTER_OFFSET = ObjectModel
      .GC_HEADER_OFFSET();
  ...
  public ObjectReference traceForwardObject(TraceLocal trace,
    ...
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(!newObject.isNull());
    return getForwardingPointer(object);
  }
   ...
   */
  public void initializeHeader(ObjectReference object) throws InlinePragma {
    // nothing to do
  }
   ...
   */
  public static boolean testAndMark(ObjectReference object) throws InlinePragma {
    Word oldValue;
    do {
   ...
   *          The object to be marked
   */
  public static boolean isMarked(ObjectReference object) throws InlinePragma {
   ...
   *          The object to be marked
   */
  public static void clearMark(ObjectReference object) throws InlinePragma {
    Word oldValue = ObjectModel.readAvailableBitsWord(object);
    ObjectModel.writeAvailableBitsWord(object, oldValue.and(GC_MARK_BIT_MASK
        .not()));
  }
  ...
  /**
   * Non-atomic clear of forwarding pointer word (assumption, thread doing the
   * set has done attempt to forward and owns the right to copy the object)
   * 
   * @param object
   *          The object whose forwarding pointer is to be set
