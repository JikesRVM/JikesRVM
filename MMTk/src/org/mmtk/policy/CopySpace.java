...
/**
 * This class implements tracing functionality for a simple copying space. Since
 * no state needs to be held globally or locally, all methods are static.
 * 
 * $Id$
 * 
 * @author Perry Cheng
 ...
 * @date $Date$
 */
public final class CopySpace extends Space implements Constants,
    Uninterruptible {
  ...
  private static final Word GC_BEING_FORWARDED = Word.one().lsh(2).minus(
      Word.one()); // ...11

  private static final Word GC_FORWARDING_MASK = GC_FORWARDED
      .or(GC_BEING_FORWARDED);
   ...
   * @param fromSpace
   *          The does this instance start life as from-space (or to-space)?
   */
   ...
   * @param markState
   *          The sense of the mark bit (flips from 0 to 1)
   */
  ...
  /**
   * Trace an object under a copying collection policy. If the object is already
   * copied, the copy is returned. Otherwise, a copy is created and returned. In
   * either case, the object will be marked on return.
   * 
   * @param trace
  ...
  public ObjectReference traceObject(TraceLocal trace, ObjectReference object)
    ...
    if (stateIsForwardedOrBeingForwarded(forwardingPtr)) {
      ...
      ObjectReference newObject = forwardingPtr.and(GC_FORWARDING_MASK.not())
          .toAddress().toObjectReference();
      return newObject;
    }
  ...
  public static void clearGCBits(ObjectReference object) throws InlinePragma {
    ...
    ObjectModel.writeAvailableBitsWord(object, header.and(GC_FORWARDING_MASK
        .not()));
  }
   ...
   */
  public static boolean isForwarded(ObjectReference object) throws InlinePragma {
   ...
   * 
   * @param object
   *          The object whose forwarding word is to be read
   * @return The forwarding word stored in <code>object</code>'s header.
   */
   ...
   * @return The forwarding pointer stored in <code>object</code>'s header.
   */
   ...
   *          The object to be marked
   * @param value
   *          The value to store in the mark bit
   */
  ...
  /**
   * Either return the forwarding pointer if the object is already forwarded (or
   * being forwarded) or write the bit pattern that indicates that the object is
   * being forwarded
   * 
   * @param object
   *          The object to be forwarded
   * @return The forwarding pointer for the object if it has already been
   *         forwarded.
   */
  private static Word attemptToForward(ObjectReference object)
    ...
    do {
      oldValue = ObjectModel.prepareAvailableBits(object);
      if (oldValue.and(GC_FORWARDING_MASK).EQ(GC_FORWARDED))
        return oldValue;
    } while (!ObjectModel.attemptAvailableBits(object, oldValue, oldValue
        .or(GC_BEING_FORWARDED)));
    return oldValue;
   ...
   */
  private static boolean stateIsBeingForwarded(Word fword) throws InlinePragma {
   ...
   */
  private static boolean stateIsForwarded(Word fword) throws InlinePragma {
  ...
  /**
   * Non-atomic write of forwarding pointer word (assumption, thread doing the
   * set has done attempt to forward and owns the right to copy the object)
   * 
   * @param object
   *          The object whose forwarding pointer is to be set
   * @param ptr
   *          The forwarding pointer to be stored in the object's forwarding
   *          word
   */
  private static void setForwardingPointer(ObjectReference object,
      ObjectReference ptr) throws InlinePragma {
    ObjectModel.writeAvailableBitsWord(object, ptr.toAddress().toWord().or(
        GC_FORWARDED));
  }
}
