...
/**
 * This class implements thread-local behavior for a reference counted space.
 * Each instance of this class captures state associated with one thread/CPU
 * acting over a particular reference counted space. Since all state is thread
 * local, instance methods of this class are not required to be synchronized.
 * 
 * @author Steve Blackburn
 * @version $Revision$
 * @date $Date$
 */
public final class RefCountLocal extends SegregatedFreeList implements
  ...
  protected final boolean preserveFreeList() {
    return true;
  }

  protected final boolean maintainInUse() {
    return true;
  }
  ...
  static {
    ...
    for (int sc = 0; sc < SIZE_CLASSES; sc++) {
      ...
      for (byte blk = 0; blk < BlockAllocator.BLOCK_SIZE_CLASSES; blk++) {
        ...
        /*
         * cells must start at multiple of MIN_ALIGNMENT because cellSize is
         * also supposed to be multiple, this should do the trick:
         */
   ...
   * 
   * @param space
   *          The ref count space with which this local thread is associated.
   * @param los
  ...
  public RefCountLocal(RefCountSpace space, LargeRCObjectLocal los,
    ...
    if (RefCountSpace.RC_SANITY_CHECK) {
      incSanityRoots = new ObjectReferenceDeque("sanity increment root set",
          incSanityRootsPool);
      sanityWorkQueue = new AddressPairDeque(sanityWorkQueuePool);
      checkSanityRoots = new ObjectReferenceDeque("sanity check root set",
          checkSanityRootsPool);
      sanityImmortalSetA = new ObjectReferenceDeque("immortal set A",
          sanityImmortalPoolA);
      sanityImmortalSetB = new ObjectReferenceDeque("immortal set B",
          sanityImmortalPoolB);
      sanityLastGCSet = new ObjectReferenceDeque("last GC set",
          sanityLastGCPool);
    }
  ...
  /**
   * Prepare the next block in the free block list for use by the free list
   * allocator. In the case of lazy sweeping this involves sweeping the
   * available cells. <b>The sweeping operation must ensure that cells are
   * pre-zeroed</b>, as this method must return pre-zeroed cells.
   * 
   * @param block
   *          The block to be prepared for use
   * @param sizeClass
   *          The size class of the block
   * @return The address of the first pre-zeroed cell in the free list for this
   *         block, or zero if there are no available cells.
   */
  ...
  public final void prepare(boolean time) {
    if (RefCountSpace.RC_SANITY_CHECK && !Options.noFinalizer.getValue())
      Assert
          .fail("Ref count sanity checks must be run with finalization disabled (-X:gc:noFinalizer=true)");
   ...
   * 
   * @param plan
   *          The plan instance performing this operation
   * @param primary
   *          Is this the thread to use for single-threaded work?
   */
  ...
  private final void processDecBufs(RCBaseMutator plan) {
    ...
    do {
      ...
      decCounter += count;
        ...
        && (RefCountSpace.RC_SANITY_CHECK || Statistics.cycles() < limit));
    decrementPhase = false;
  }

  /**
   * Process the root buffers from the previous GC, if the object is no longer
   * live release it.
   * 
   * @param plan
   *          The plan instance performing this operation
  ...
  /**
   * Process the root buffers, moving entries over to the decrement buffers for
   * the next GC.
   */
  ...
  /**
   * Decrement the reference count of an object. If the count drops to zero, the
   * release the object, performing recursive decremetns and freeing the object.
   * If not, then if cycle detection is being used, record this object as the
   * possible source of a cycle of garbage (all non-zero decrements are
   * potential sources of new cycles of garbage.
   * 
   * @param object
   *          The object whose count is to be decremented
   * @param plan
   *          The plan instance performing this operation
   */
  ...
  /**
   * An object is dead, so before freeing it, scan the object for recursive
   * decrement (each outgoing pointer from this dead object is now dead, so the
   * targets must have their counts decremented).
   * <p>
   * 
   * If the object is being held in a buffer by the cycle detector, then the
   * object must not be freed. It will be freed later when the cycle detector
   * processes its buffers.
   * 
   * @param object
  ...
  private final void release(ObjectReference object, RCBaseMutator plan)
    ...
    if (!RCBase.REF_COUNT_CYCLE_DETECTION || !RefCountSpace.isBuffered(object))
      free(object);
  }

  /**
   * Free an object. First determine whether it is managed by the LOS or the
   * regular free list. If managed by LOS, delegate freeing to the LOS.
   * Otherwise, establish the cell, block and sizeclass for this object and call
   * the free method of our subclass.
   * 
   * @param object
   *          The object to be freed.
   */
  public final void free(ObjectReference object) throws InlinePragma {
   ...
   * 
   * Sanity check code allows reference counts to be cross-checked with counts
   * established via a transitive closure. The code has two significant
   * limitations:
   *  . Finalization is not supported---it must be turned off to avoid anomalies
   * relating to finalization's odd reachability semantics.
   *  . Currently immortal (and boot image) objects are uncollected. If any such
   * object were to become unreachable, decrements would not be issued for any
   * referent RC objects---leading to a discrepancy between RC and sanity RC
   * counts.
   * 
   * To maximize the utility of this mechanism in the face of the above
   * problems, it is best to trigger frequent GCs by setting the metadata limit
   * to its minimum.
   */

  /**
   * Add an entry to the root buffer for the increment sanity traversal. (used
   * only for sanity checks).
   * 
   * @param object
   *          The object to be added to the root buffer
   */
  ...
  /**
   * Add an entry to the sanity traversal work queue (the work queue is used
   * instead of a stack in establishing the transitive closure). (used only for
   * sanity checks).
   * 
   * @param object
   *          The object to be added to the work queue buffer
   * @param location
   *          The location from which the object is reached
   */
  public final void sanityTraceEnqueue(ObjectReference object, Address location) {
    sanityWorkQueue.push(object.toAddress(), location);
  }

  /**
   * Perform a sanity increment traversal. This involves starting from roots and
   * performing a transitive closure, incrementing the sanity reference count of
   * each object each time it is visited.
   */
  ...
  /**
   * Perform a sanity check traversal. This involves starting from roots and
   * performing a transitive closure, checking the sanity reference count of
   * each object against its actual reference count, failing with an error if
   * there is a mismatch. A check is also made of the number of live objects
   * (comparing RC and tracing).
   */
  final void checkSanityTrace() {
    ...
    while (!(object = checkSanityRoots.pop()).isNull()) {
      if (Statistics.getCollectionCount() == 1)
        checkForImmortal(object);
      RCBase.collector().checkSanityTrace(object, Address.zero());
    }
    while (!(object = sanityWorkQueue.pop1().toObjectReference()).isNull()) {
      if (Statistics.getCollectionCount() == 1)
    ...
    if (rcLiveObjects != sanityLiveObjects) {
      Log.write("live mismatch: ");
      Log.write(rcLiveObjects);
      Log.write(" (rc) != ");
      Log.write(sanityLiveObjects);
   ...
   * 
   * @param purple
   *          The new value for the purple counter.
   */
  ...
  /**
   * Print out statistics on increments, decrements, roots and potential garbage
   * cycles (purple objects).
   */
  public final void printStats() {
    Log.write("<GC ");
    Log.write(Stats.gcCount());
    Log.write(" ");
    Log.write(incCounter);
    Log.write(" incs, ");
    Log.write(decCounter);
    Log.write(" decs, ");
    Log.write(rootCounter);
    Log.write(" roots");
    if (RCBase.REF_COUNT_CYCLE_DETECTION) {
      Log.write(", ");
      Log.write(purpleCounter);
      Log.write(" purple");
    }
    Log.writeln(">");
  }
