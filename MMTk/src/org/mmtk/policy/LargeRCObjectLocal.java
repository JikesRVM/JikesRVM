 ...
 * access to reference-counted large object. Therefore instances must not be
 * shared across truely concurrent threads (CPUs). Rather, one or more instances
 * of this class should be bound to each CPU. The shared VMResource used by each
 * instance is the point of global synchronization, and synchronization only
 * occurs at the granularity of aquiring (and releasing) chunks of memory from
 * the VMResource.
 * 
 * If there are C CPUs and T LargeObjectSpaces, there must be C X T instances of
 * this class, one for each CPU, LargeObjectSpace pair.
 * 
 * $Id$
 * 
 ...
 * @date $Date$
 */
public final class LargeRCObjectLocal extends LargeObjectAllocator implements
   ...
   * 
   * @param space
   *          The large object space to which this thread instance is bound.
   */
