...
/**
 * This class implements the global state of a full-heap collector with a
 * copying nursery and mark-sweep mature space. Unlike a full generational
 * collector, there is no write barrier, no remembered set, and every collection
 * is full-heap.
 * 
  ...
  public final void collectionPhase(int phaseId) throws InlinePragma {
    ...
    if (phaseId == RELEASE) {
      ...
      if (progress) {
        ...
        if (threshold < CopyMS_PAGE_RESERVE)
          threshold = CopyMS_PAGE_RESERVE;
