...
/**
 * This abstract class implements the global core functionality for all memory
 * management schemes. All global MMTk plans should inherit from this class.
 * <p>
 * 
 ...
 * understanding the correctness and performance proprties of MMTk plans.
 * 
 * $Id$
 * 
 * @author Perry Cheng
...
public abstract class Plan implements Uninterruptible, Constants {
  ...
  public static final ImmortalSpace immortalSpace = new ImmortalSpace(
      "immortal", DEFAULT_POLL_FREQUENCY, IMMORTAL_MB);

  /**
   * All meta data that is used by MMTk is allocated (and accounted for) in the
   * meta data space.
   */
  public static final RawPageSpace metaDataSpace = new RawPageSpace("meta",
      DEFAULT_POLL_FREQUENCY, META_DATA_MB);

  /** Large objects are allocated into a special large object space. */
  public static final LargeObjectSpace loSpace = new LargeObjectSpace("los",
      DEFAULT_POLL_FREQUENCY, LOS_FRAC);
  ...
  /**
   * The postBoot method is called by the runtime immediately after command-line
   ...
   * order to parse the command line arguments. For this reason all plans should
   * operate gracefully on the default minimum heap size until the point that
   * boot is called.
   */
  ...
  public void notifyExit(int value) {
    ...
    if (Options.verboseTiming.getValue())
      printDetailedTiming(true);
  }

  /**
   * Any Plan can override this to provide additional plan specific timing
   * information.
   * 
   * @param totals
   *          Print totals
  ...
  public Word setBootTimeGCBits(Address ref, ObjectReference typeRef, int size,
      Word status) throws InlinePragma {
   ...
   */
  public final boolean stressTestGCRequired() throws InlinePragma {
   ...
   * <p>
   * 
   * This is decoupled from the poll() mechanism because the triggering of
   * asynchronous collections can trigger write barriers, which can trigger an
   * asynchronous collection. Thus, if the triggering were tightly coupled with
   * the request to alloc() within the write buffer code, then inifinite regress
   * could result. There is no race condition in the following code since there
   * is no harm in triggering the collection more than once, thus it is
   * unsynchronized.
   */
  ...
  /**
   * A collection has been initiated. Increment the collectionInitiated state
  ...
  /**
   * A collection has fully completed. Decrement the collectionInitiated state
   * variable appropriately.
   */
  ...
  /**
   * A user-triggered GC has been initiated. By default, do nothing, but this
   * may be overridden.
   */
  ...
  /**
   * Return the name of the space into which an allocator is allocating. The
   * allocator, <code>a</code> may be assocaited with any plan instance.
   * 
   * @param a
   *          An allocator
   * @return The name of the space into which <code>a</code> is allocating, or "<null>"
   *         if there is no space associated with <code>a</code>.
   */
  ...
  /**
   * Return the space into which an allocator is allocating. The allocator,
   * <code>a</code> may be assocaited with any plan instance.
   * 
   * @param a
   *          An allocator
  ...
  /**
   * Generic hook to allow benchmarks to be harnessed. A plan may use this to
   * perform certain actions prior to the commencement of a benchmark, such as a
   * full heap collection, turning on instrumentation, etc. By default we do a
   * full heap GC, and then start stats collection.
   */
  ...
  /**
   * Generic hook to allow benchmarks to be harnessed. A plan may use this to
   * perform certain actions after the completion of a benchmark, such as a full
   * heap collection, turning off instrumentation, etc. By default we stop all
   * statistics objects and print their values.
   */
  ...
  /**
   * Return the amount of <i>free memory</i>, in bytes (where free is defined
   * as not in use). Note that this may overstate the amount of <i>available
   * memory</i>, which must account for unused memory that is held in reserve
  ...
  /**
   * Return the amount of <i>memory in use</i>, in bytes. Note that this
   * excludes unused memory that is held in reserve for copying, and therefore
  ...
  /**
   * Return the amount of <i>memory in use</i>, in bytes. Note that this
   * includes unused memory that is held in reserve for copying, and therefore
   * unavailable for allocation.
   * 
   * @return The amount of <i>memory in use</i>, in bytes.
   */
  ...
  /**
   * Return the total amount of memory managed to the memory management system,
   * in bytes.
   * 
   * @return The total amount of memory managed to the memory management system,
   *         in bytes.
   ...
   * Sub-classes must override the getCopyReserve method, as the arithmetic here
   * is fixed.
   * 
  ...
  /**
   * Return the number of pages reserved for copying. Subclasses that manage a
   * copying space must add the copying contribution.
   * 
   * @return The number of pages reserved given the pending allocation,
   *         including space reserved for copying.
   */
   ...
   * 
   * @return The time cap for this GC (i.e. the time by which it should
   *         complete).
   */
  ...
  /**
   * This method is called periodically by the allocation subsystem (by default,
   * each time a page is consumed), and provides the collector with an
   * opportunity to collect.
   * 
   * @param mustCollect
   *          Should a collection be forced.
   * @param space
   ...
   * @param wait
   *          Should we wait for a client to connect?
   */
