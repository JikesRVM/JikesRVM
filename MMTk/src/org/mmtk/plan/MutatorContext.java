 ...
 * behavior. We assume <i>N</i> collector threads and <i>M</i> mutator
 * threads, where <i>N</i> is often equal to the number of available
 * processors, P (for P-way parallelism at GC-time), and <i>M</i> may simply be
 * the number of mutator (application) threads. Both <i>N</i> and <i>M</i> are
 * determined by the VM, not MMTk. In the case where a VM uses posix threads
 * (pthreads) for each mutator ("1:1" threading), <i>M</i> will typically be
 * equal to the number of mutator threads. When a uses "green threads" or a
 * hybrid threading scheme (such as Jikes RVM), <i>M</i> will typically be
 * equal to the level of <i>true</i> parallelism (ie the number of underlying
 * kernel threads).
 * <p>
 * 
 ...
 * 
 * MMTk explicitly separates thread-local (this class) and global operations
 * (@see Plan), so that syncrhonization is localized and explicit, and thus
 * hopefully minimized (@see Plan). Gloabl (Plan) and per-thread (this class)
 * state are also explicitly separated. Operations in this class (and its
 * children) are therefore strictly local to each mutator thread, and
 * synchronized operations always happen via access to explicitly global classes
 * such as Plan and its children. Therefore only <i>"fast path"</i>
 * (unsynchronized) allocation and barrier semantics are defined in
 * MutatorContext and its subclasses. These call out to <i>"slow path"</i>
 * (synchronize(d) methods which have global state and are globally
 * synchronized. For example, an allocation fast path may bump a pointer without
 * any syncrhonization (the "fast path") until a limit is reached, at which
 * point the "slow path" is called, and more memory is aquired from a global
 * resource.
 * <p>
 * 
 * As the super-class of all per-mutator contexts, this class implements basic
 * per-mutator behavior common to all MMTk collectors, including support for
 * immortal and large object space allocation, as well as empty stubs for write
 * barriers (to be overridden by sub-classes as needed).
 * 
...
public abstract class MutatorContext implements Uninterruptible, Constants {
   ...
   * (worst) method of aligning to determine if the object is a large object to
   * ensure that no objects are larger than other allocators can handle.
   * 
   * @param bytes
  ...
  public void postAlloc(ObjectReference ref, ObjectReference typeRef,
    ...
    switch (allocator) {
    case Plan.ALLOC_LOS:
      Plan.loSpace.initializeHeader(ref);
      return;
    case Plan.ALLOC_IMMORTAL:
      Plan.immortalSpace.postAlloc(ref);
      return;
    default:
      if (Assert.VERIFY_ASSERTIONS)
        Assert.fail("No such allocator");
  ...
  /**
   * Given an allocator, <code>a</code>, determine the space into which
   * <code>a</code> is allocating and then return an allocator (possibly
   * <code>a</code>) associated with <i>this plan instance</i> which is
   * allocating into the same space as <code>a</code>.
   * <p>
   * 
   * The need for the method is subtle. The problem arises because application
   * threads may change their affinity with processors/posix threads, and this
   * may happen during a GC (at the point at which the scheduler performs thread
   * switching associated with the GC). At the end of a GC, the thread that
   * triggered the GC may now be bound to a different processor and thus the
   * allocator instance on its stack may be no longer be valid (i.e. it may
   * pertain to a different plan instance).
   * <p>
   * 
   * This method allows the correct allocator instance to be established and
   * associated with the thread (see {@link
   * org.mmtk.utility.alloc.Allocator#allocSlowBody(int, int, int, boolean)
   * Allocator.allocSlowBody()}).
   * 
   * @see org.mmtk.utility.alloc.Allocator
   * @see org.mmtk.utility.alloc.Allocator#allocSlowBody(int, int, int, boolean)
   * 
   * @param a
   *          An allocator instance.
   * @return An allocator instance associated with <i>this plan instance</i>
   *         that allocates into the same space as <code>a</code> (this may in
   *         fact be <code>a</code>).
   */
  ...
  /*****************************************************************************
   * 
   * Write and read barriers. By default do nothing, override if appropriate.
   */
   ...
   * @param metaDataA
   *          An int that assists the host VM in creating a store
   * @param metaDataB
   *          An int that assists the host VM in creating a store
   * @param mode
   *          The context in which the store occured
   */
  ...
  /**
   * Read a reference. Take appropriate read barrier action, and return the
   * value that was read.
   * <p>
   * This is a <b>substituting<b> barrier. The call to this barrier takes the
   * place of a load.
   * <p>
   * 
   * @param src
   ...
   * @param context
   *          The context in which the read arose (getfield, for example)
  ...
  public int getId() throws InlinePragma {
    return id;
  }
}
