 ...
 * for <i>generational copying collectors</i>.
 * <p>
 * 
...
public class GenMutator extends StopTheWorldMutator implements Uninterruptible {
   ...
   * Constructor
   * 
   * Note that each mutator is a producer of remsets, while each collector is a
   * consumer. The <code>GenCollector</code> class is responsible for
   ...
   * @param metaDataA
   *          A field used by the VM to create a correct store.
   * @param metaDataB
   *          A field used by the VM to create a correct store.
   * @param mode
   ...
   * <p>
   * 
   * In this case, we remember the mutated source address range and will scan
   * that address range at GC time.
   * 
   * @param src
  ...
  public final boolean writeBarrier(ObjectReference src, Offset srcOffset,
    ...
    if (dst.toAddress().LT(Gen.NURSERY_START))
      arrayRemset.insert(dst.toAddress().plus(dstOffset), dst.toAddress().plus(
          dstOffset.plus(bytes)));
    return false;
  }
