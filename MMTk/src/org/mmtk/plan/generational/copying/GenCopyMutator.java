...
public class GenCopyMutator extends GenMutator implements Uninterruptible {
  ...
  /**
   * The allocator for the copying mature space (the mutator may "pretenure"
   * objects into this space otherwise used only by the collector)
   */
   ...
   *          The allocator to allocate from
   * @param bytes
   *          The size of the space allocated (in bytes)
   */
  ...
  public final Allocator getAllocatorFromSpace(Space space) {
    if (space == GenCopy.matureSpace0 || space == GenCopy.matureSpace1)
      return mature;
