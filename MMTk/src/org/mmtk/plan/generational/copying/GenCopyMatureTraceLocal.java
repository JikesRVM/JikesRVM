 ...
 * over the heap graph, specifically in a Generational copying collector.
 * 
 * $Id$
 * 
 ...
 * @date $Date$
 */
public class GenCopyMatureTraceLocal extends GenMatureTraceLocal implements
    Uninterruptible {
   ...
   * determining whether the instance is in from space, and if so, calling the
   * <code>traceObject</code> method of the Copy collector.
   * 
   * @param object
  ...
  public final ObjectReference traceObject(ObjectReference object) {
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(global().traceFullHeap());
    if (object.isNull())
      return object;
