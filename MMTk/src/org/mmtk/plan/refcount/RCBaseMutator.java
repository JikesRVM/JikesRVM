...
/**
 * This abstract class implements <i>per-mutator thread</i> behavior and state
 * for the <i>RCBase</i> plan, which is the parent class for reference counting
 * collectors.
 * <p>
 * 
 ...
 * behaviors, so most of the collection logic here should really be
 ...
 * @date $Date$
 */
public abstract class RCBaseMutator extends StopTheWorldMutator implements
    Uninterruptible {
  /*****************************************************************************
   * Instance fields
   */
   ...
   * <p>
   * 
   * Note that we override <code>los</code>, so we must account for it here
   * (rather than in our superclass).
   * 
  ...
  public Space getSpaceFromAllocator(Allocator a) {
    if (a == rc)
      return RCBase.rcSpace;
    if (a == los)
      return RCBase.loSpace;
  ...
  /**
   * A field of an object is being enumerated by ScanObject as part of a
   * recursive decrement (when an object dies, its referent objects must have
   * their counts decremented). If the field points to the RC space, decrement
   * the count for the referent.
   * 
   * @param location
  ...
  /*
   * public void enumerateModifiedPointerLocation(Address objLoc) throws
   * InlinePragma { if (Assert.VERIFY_ASSERTIONS)
   * Assert._assert(RCBase.WITH_COALESCING_RC); ObjectReference object =
   * objLoc.loadObjectReference(); if (RCBase.isRCObject(object))
   ...
   * 
   * @param object
   *          The object to be added to the decrement buffer
   */
  public final void addToDecBuf(ObjectReference object) throws InlinePragma {
    decBuffer.push(object);
  }
   ...
   * 
   * @param root
   *          The object to be added to root set
   */
  public final void addToRootSet(ObjectReference root) throws InlinePragma {
    newRootSet.push(root);
  }

  /**
   * Process the modified object buffers, enumerating each object's fields
   */
  ...
  /**
   * Trace a reference during an check sanity traversal. This is only used as
   ...
   *          applicable.
   */
  public void checkSanityTrace(ObjectReference object, Address location) {
    if (RCBase.isRCObject(object)) {
