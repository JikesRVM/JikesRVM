 ...
 * <p>
 * 
 * Specifically, this class defines mutation-time allocation (allocation into
 * the nursery and "pre-tenuring" into the mature space), write barriers and
 * per-mutator collection semantics (such as flushing rememberd sets flushing
 * and initializing allocators).
 * <p>
 * 
 ...
 * @date $Date$
 */
public class GenRCMutator extends RCBaseMutator implements Uninterruptible,
    Constants {
   ...
   * superclasses appropriately. This exists to support
   * {@link org.mmtk.plan.CollectorContext#getOwnAllocator(Allocator)}.
   * 
  ...
  public final void writeBarrier(ObjectReference src, Address slot,
      ...
      throws InlinePragma {
    if (GenRC.GATHER_WRITE_BARRIER_STATS)
      GenRC.wbFast.inc();
  ...
  /**
   * This object <i>may</i> need to be logged because we <i>may</i> have been
   * the first to update it. We can't be sure because of the (delibrate) lack of
   * synchronization in the <code>logRequired()</code> method, which can
   * generate a race condition. So, we now use an atomic operation to arbitrate
   * the race. If we successful, we will log the object, enumerating its
  ...
  /**
   * A field of an object rememebered in the modified objects buffer is being
   * enumerated by ScanObject. If the field points to the nursery, then add the
   * field address to the locations buffer. If the field points to the RC space,
   * increment the count of the referent object.
   * 
   * @param objLoc
  ...
  public final void enumerateModifiedPointerLocation(Address objLoc)
    ...
    if (!object.isNull()) {
      if (Space.isInSpace(GenRC.NS, object)) {
        ...
        ((GenRCCollector) ActivePlan.collector()).trace
            .traceObjectLocation(objLoc);
