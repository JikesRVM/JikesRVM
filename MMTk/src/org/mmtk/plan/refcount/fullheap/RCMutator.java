 ...
 * <p>
 * 
 * Specifically, this class implements mutator-time allocation and write barrier
 * semantics. Allocation and write barriers are all performed with respect to
 * thread local storage, to maximize locality and minimze synchronization.
 * <p>
 * 
 ...
 * @date $Date$
 */
public class RCMutator extends RCBaseMutator implements Uninterruptible,
    Constants {
  /*****************************************************************************
   * 
  ...
  public void postAlloc(ObjectReference object, ObjectReference typeRef,
      int bytes, int allocator) throws NoInlinePragma {
    switch (allocator) {
    case RC.ALLOC_RC:
  ...
  /**
   * A new reference is about to be created. Perform appropriate write barrier
   * action.
   * <p>
   * 
   * In this case, we remember the address of the source of the pointer if the
   * new reference points into the nursery from non-nursery space. This method
   * is <b>inlined</b> by the optimizing compiler, and the methods it calls are
   * forced out of line.
   * 
   * @param src
   ...
   *          An int that assists the host VM in creating a store
   * @param mode
   *          The mode of the store (eg putfield, putstatic)
   */
   ...
   * @param slot
   *          The address of the word (slot) being mutated.
   * @param tgt
   *          The target of the new reference (about to be stored into src).
   * @param metaDataA
   *          An int that assists the host VM in creating a store
   * @param metaDataB
   *          An int that assists the host VM in creating a store
  ...
  private final void writeBarrierInternal(ObjectReference src, Address slot,
      ObjectReference tgt, Offset metaDataA, int metaDataB, int mode)
      throws InlinePragma {
    if (RC.GATHER_WRITE_BARRIER_STATS)
      RC.wbFast.inc();
    if (RC.WITH_COALESCING_RC) {
      ...
      Barriers
          .performWriteInBarrier(src, slot, tgt, metaDataA, metaDataB, mode);
    } else {
      ObjectReference old = Barriers.performWriteInBarrierAtomic(src, slot,
          tgt, metaDataA, metaDataB, mode);
      if (RC.isRCObject(old))
        decBuffer.pushOOL(old);
      if (RC.isRCObject(tgt))
        RefCountSpace.incRCOOL(tgt);
    }
  }

  /**
   * An out of line version of the write barrier. This method is forced <b>out
   * of line</b> by the optimizing compiler, and the methods it calls are
   * forced out of inline.
   * 
   * @param src
   *          The object being mutated.
   * @param slot
   ...
   * <p>
   * 
   * In this case, we simply remember the mutated source object, or we enumerate
   * the copied pointers and perform appropriate actions on each.
   * 
   * @param src
   *          The source of the values to copied
   * @param srcOffset
   ...
   *         the caller (this depends on which style of barrier is being used).
   */
  public boolean writeBarrier(ObjectReference src, Offset srcOffset,
      ObjectReference dst, Offset dstOffset, int bytes) throws InlinePragma {
    if (RC.GATHER_WRITE_BARRIER_STATS)
      RC.wbFast.inc();
    if (RC.WITH_COALESCING_RC) {
   ...
   * each referent object (in the RC space) before setting the header bit to
   * indicate that it has finished logging (allowing others in the race to
   * continue).
   * 
   * @param srcObj
   *          The object being mutated
  ...
  private final void coalescingWriteBarrierSlow(ObjectReference srcObj)
    ...
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(RC.WITH_COALESCING_RC);
    if (RC.GATHER_WRITE_BARRIER_STATS)
      RC.wbSlow.inc();
