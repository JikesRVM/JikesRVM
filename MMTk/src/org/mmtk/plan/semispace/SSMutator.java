 ...
 * <p>
 * 
 * Specifically, this class defines <i>SS</i> mutator-time allocation and
...
public class SSMutator extends StopTheWorldMutator implements Uninterruptible {
   ...
   * {@link CollectorContext#getOwnAllocator(Allocator)}.
   * 
  ...
  public final Allocator getAllocatorFromSpace(Space space) {
    if (space == SS.copySpace0 || space == SS.copySpace1)
      return ss;
    return super.getAllocatorFromSpace(space);
  }

  /**
   * Give the compiler/runtime statically generated alloction advice which will
   * be passed to the allocation routine at runtime.
   * 
   * @param type
   *          The type id of the type being allocated
   * @param bytes
   *          The size (in bytes) required for this object
   * @param callsite
   *          Information identifying the point in the code where this
   *          allocation is taking place.
   * @param hint
   *          A hint from the compiler as to which allocator this site should
   *          use.
   * @return Allocation advice to be passed to the allocation routine at runtime
   */
