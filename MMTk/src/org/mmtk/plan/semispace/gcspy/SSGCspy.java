 ...
 * It makes use of Daniel Frampton's LinearScan patches to allow CopySpaces to
 * be scanned linearly (rather than recording allocations with an ObjectMap as
 * in prior versions.
 * <p>
 * 
 * See the Jones & Lins GC book, section 2.2 for an overview of the basic
 * algorithm. This implementation also includes a large object space (LOS), and
 * an uncollected "immortal" space.
 * <p>
 * 
...
public class SSGCspy extends SS implements Uninterruptible {
   ...
   *          Whether to wait
   * @param port
   *          The port to talk to the GCspy client (e.g. visualiser)
   */
  public final void startGCspyServer(int port, boolean wait)
      throws InterruptiblePragma {
    String eventNames[] = { "Before collection", "Semispace copied",
        "After collection" };
    String generalInfo = "SemiSpace Server Interpreter\n\nGeneral Info";

    // The Server
    ServerInterpreter.init("SemiSpaceServerInterpreter", port, eventNames,
        true /* verbose */, generalInfo);
    ...
    ss0Driver = new ContiguousSpaceDriver("Semispace 0 Space", copySpace0,
        tilesize, true);
    ss1Driver = new ContiguousSpaceDriver("Semispace 1 Space", copySpace1,
        tilesize, false);
    immortalDriver = new ContiguousSpaceDriver("Immortal Space", immortalSpace,
        tilesize, false);
    losDriver = new TreadmillDriver("Large Object Space", loSpace, tilesize,
        LOS_SIZE_THRESHOLD, false);
  ...
  private static void reportSpaces() {
    Log.write("  Low semispace:  ");
    Log.write(copySpace0.getStart());
    // Log.writeln("-", copySpace0.getCursor());
    Log.write("  High semispace: ");
    Log.write(copySpace1.getStart());
