 ...
 * state and write buffer/remset state). <code>SimplePhase</code> ensures that
 * per-collector thread GC phases are performed by each collector thread, and
 * that the <i>M</i> per-mutator thread operations are multiplexed across the
 * <i>N</i> active collector threads (@see SimplePhase#delegatePhase).
 * <p>
 * 
 ...
 * this state are therefore assumed to be low-cost at GC time.
 * <p>
 * 
 ...
 * children) are therefore strictly local to each collector thread, and
 * synchronized operations always happen via access to explicitly global classes
 * such as Plan and its children.
 * <p>
 * 
...
public abstract class CollectorContext implements Uninterruptible, Constants {
   ...
   * @param bytes
   *          The number of bytes required for the copy.
   * @param align
   *          Required alignment for the copy.
   * @param offset
   ...
   */
  public void postCopy(ObjectReference ref, ObjectReference typeRef, int bytes,
      int allocator) {
   ...
   */
  public int copyCheckAllocator(ObjectReference from, int bytes, int align,
