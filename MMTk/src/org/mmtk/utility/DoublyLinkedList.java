...
/**
 * FIXME This class must be re-written as it makes the assumption that the
 * implementation language (Java) and the language being implemented are the
 * same. This is true in the case of Jikes RVM, but it is not true for any VM
 * implementing a language other than Java.
 * 
 * 
...
final class DoublyLinkedList implements Constants, Uninterruptible {
  ...
  private final int log_granularity; // Each node on the treadmill is
  ...
  private static final Offset PREV_OFFSET = Offset
      .fromIntSignExtend(0 * BYTES_IN_ADDRESS);

  private static Offset NEXT_OFFSET = Offset
      .fromIntSignExtend(1 * BYTES_IN_ADDRESS);

  private static Offset LIST_OFFSET = Offset
      .fromIntSignExtend(2 * BYTES_IN_ADDRESS);

  private static Offset HEADER_SIZE = Offset
      .fromIntSignExtend(3 * BYTES_IN_ADDRESS);

  private static final Word nodeMask;
  static {
    Word mask = Word.one();
    while (mask.LT(HEADER_SIZE.plus(MAX_BYTES_PADDING).toWord()))
      mask = mask.lsh(1);
  ...
  public final boolean isNode(Address node) {
    return node.toWord().rshl(log_granularity).lsh(log_granularity).EQ(
        node.toWord());
  }
  ...
  static public final Address midPayloadToNode(Address payload)
      throws InlinePragma {
    // This method words as long as you are less than MAX_BYTES_PADDING into the
    // payload.
