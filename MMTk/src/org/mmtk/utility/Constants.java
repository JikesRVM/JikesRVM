...
/**
 * MMTk follows the pattern set by Jikes RVM for defining sizes of primitive
 * types thus:
 * 
 * static final int LOG_BYTES_IN_INT = 2; static final int BYTES_IN_INT = 1<<LOG_BYTES_IN_INT;
...
public interface Constants {
  ...
  static final int LOG_BITS_IN_ADDRESS = LOG_BITS_IN_BYTE
  ...
  /**
   * This value specifies the <i>minimum</i> allocation alignment requirement
   * of the VM. When making allocation requests, both <code>align</code> and
   * <code>offset</code> must be multiples of <code>MIN_ALIGNMENT</code>.
   * 
  ...
  /**
   * The maximum alignment request the vm will make. This must be a power of two
   * multiple of the minimum alignment.
   */
  static final int MAX_ALIGNMENT = MIN_ALIGNMENT << VMConstants
      .MAX_ALIGNMENT_SHIFT();

  /**
   * The VM will add at most this value minus BYTES_IN_INT bytes of padding to
   * the front of an object that it places in a region of memory. This value
