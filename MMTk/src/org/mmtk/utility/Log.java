...
public class Log implements Constants, Uninterruptible {
  ...
  /**
   * characters in the write buffer for the caller's message. This does not
   * include characters reserved for the overflow message.
   * 
   * This needs to be large because Jikes RVM's implementation of Lock.java logs
   * a lot of information when there is potential GC deadlock.
   */
  ...
  private static final char OVERFLOW_MESSAGE_FIRST_CHAR = OVERFLOW_MESSAGE
      .charAt(0);
  ...
  /**
   * characters in buffer for building string representations of longs. A long
   * is a signed 64-bit integer in the range -2^63 to 2^63+1. The number of
   * digits in the decimal representation of 2^63 is ceiling(log10(2^63)) ==
   * ceiling(63 * log10(2)) == 19. An extra character may be required for a
   * minus sign (-). So the maximum number of characters is 20.
   */
  ...
  /**
   * log2 of number of digits in the unsigned hexadecimal representation of a
   * byte
   */
  private static final int LOG_HEX_DIGITS_IN_BYTE = LOG_BITS_IN_BYTE
      - LOG_BITS_IN_HEX_DIGIT;

  /**
   * map of hexadecimal digit values to their character representations
   */
  private static final char[] hexDigitCharacter = { '0', '1', '2', '3', '4',
  ...
  /**
   * The last character that was written by #addToBuffer(char). This is used to
   * check whether we want to newline-terminate the text.
   */
  ...
  /**
   * writes a long, in decimal. The value is not padded and no thousands
   * seperator is logged. If the value is negative a leading minus sign (-) is
   * logged.
   * 
   * @param l
   *          long value to be logged
   */
  ...
  /**
   * writes a <code>double</code>. Two digits after the decimal point are
   * always logged. The value is not padded and no thousands seperator is used.
   * If the value is negative a leading hyphen-minus (-) is logged. The decimal
   * point is a full stop (.).
   * 
   * @param d
  ...
  /**
   * writes a <code>double</code>. The number of digits after the decimal
   * point is determined by <code>postDecimalDigits</code>. The value is not
   * padded and not thousands seperator is used. If the value is negative a
   * leading hyphen-minus (-) is logged. The decimal point is a full stop (.)
   * and is logged even if <postDecimcalDigits</code> is zero. If <code>d</code>
   * is greater than the largest representable value of type <code>int</code>,
   * it is logged as "TooBig". Similarly, if it is less than the negative of the
   * largest representable value, it is logged as "TooSmall". If <code>d</code>
   * is NaN is is logged as "NaN".
   * 
   * @param d
   *          the double to be logged
   * @param postDecimalDigits
   *          the number of digits to be logged after the decimal point. If less
   *          than or equal to zero no digits are logged, but the decimal point
   *          is.
   */
   ...
   * 
   * @param c
   *          the array of characters to be logged
   */
  public static void write(char[] c) {
    write(c, c.length);
  }
   ...
   * @param len
   *          the number of characters to be logged, starting with the first
   *          character
   */
   ...
   * 
   * @param b
   *          the array of bytes to be logged
   */
   ...
   * 
   * @param o
   *          the object reference to be logged
   */
  ...
  /**
   * writes the start of an array of characters and a new-line, then flushes the
   * buffer.
   * 
   * @see #write(char [], int)
   * 
  ...
  /**
   * writes an object reference, in hexademical, and a new-line, then flushes
   * the buffer.
   * 
   ...
   *          boolean value to be logged.
   * @param flush
   *          if <code>true</code> then flushes the buffer
   */
  ...
  /**
   * writes a long, in decimal, and a new-line, then optionally flushes the
   * buffer.
   * 
   * @see #write(long)
   * 
  ...
  /**
   * writes a <code>double</code> and a new-line, then optionally flushes the
   * buffer.
   * 
   * @see #write(double)
   * 
   ...
   *          the double to be logged
   * @param flush
   *          if <code>true</code> then flush the buffer
   */
  ...
  /**
   * writes an array of characters and a new-line, then optionally flushes the
   * buffer.
   * 
   * @see #write(char [])
   * 
  ...
  /**
   * writes the start of an array of characters and a new-line, then optionally
   * flushes the buffer.
   * 
   * @see #write(char [], int)
   * 
  ...
  /**
   * writes a word, in hexadecimal, and a new-line, then optionally flushes the
   * buffer.
   * 
   * @see #write(Word)
   * 
  ...
  /**
   * writes an address, in hexademical, and a new-line, then optionally flushes
   * the buffer.
   * 
   * @see #write(Address)
   * 
  ...
  /**
   * writes an object reference, in hexademical, and a new-line, then optionally
   * flushes the buffer.
   * 
   * @see #write(ObjectReference)
   * 
  ...
  /**
   * writes an offset, in hexademical, and a new-line, then optionally flushes
   * the buffer.
   * 
   * @see #write(Offset)
   * 
  ...
  /**
   * writes an extent, in hexademical, and a new-line, then optionally flushes
   * the buffer.
   * 
   * @see #write(Extent)
   * 
  ...
  /**
   * flushes the buffer. The buffered effected of writes since the last flush
   * will be logged in one block without output from other thread's logging
   * interleaving.
   */
   ...
   * 
   * @param flush
   *          if <code>true</code> the buffer is flushed
   */
   ...
   *          the Word to be logged
   * @param bytes
   *          the number of bytes from the long to be logged. If less than 8
   *          then the least significant bytes are logged and some of the most
   *          significant bytes are ignored.
   */
  ...
  /**
   * sets the flag so that a thread identifier will be included before the
   * logged message
   */
  ...
  /**
   * gets the buffer for building string representations of integers. There is
   * one of these buffers for each Log instance.
   */
