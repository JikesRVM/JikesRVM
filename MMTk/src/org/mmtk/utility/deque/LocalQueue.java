...
/**
 * This class implements a local (<i>unsynchronized</i>) queue. A queue is
 * strictly FIFO.
 * <p>
 * 
 ...
 * enqueued at the tail of a <code>SharedDeque</code>.
 * 
 * The implementation is intended to be as efficient as possible, in time and
 * space, and is the basis for the <code>TraceBuffer</code> used by heap trace
 * generation. Each instance adds a single field to those inherited from the
 * SSB: a bump pointer.
 * 
 ...
 * packed to the low end of the buffer. Thus buffer underflows will always arise
...
class LocalQueue extends LocalSSB implements Constants, Uninterruptible {
  ...
  /**
   * Check whether there are values in the buffer for a pending dequeue. If
   * there is not data, grab the first buffer on the shared queue (after freeing
   * the buffer).
   * 
   * @param arity
   *          The arity of the values stored in this queue: the buffer must
  ...
  protected final boolean checkDequeue(int arity) throws InlinePragma {
    if (bufferOffset(head).isZero()) {
      ...
      if (Assert.VERIFY_ASSERTIONS)
        Assert
            ._assert(bufferOffset(head).sGE(
                Word.fromIntZeroExtend(arity).lsh(LOG_BYTES_IN_ADDRESS)
                    .toOffset()));
      return true;
    }
  }
  ...
  /**
   * Dequeue a value from the buffer. This is <i>unchecked</i>. The caller must
   * first call <code>checkDequeue()</code> to ensure the buffer has and entry
   * to be removed.
   * 
   * @return The first entry on the queue.
   */
  protected final Address uncheckedDequeue() throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(bufferOffset(head).sGE(
   ...
   * If the tail has sufficient entries, consume the tail. Otherwise try wait on
   * the global queue until either all other clients of the queue reach
   * exhaustion or a buffer becomes available.
   * 
   * @param arity
   ...
   * Acquire a new head buffer. If the shared queue has no buffers available,
   * consume the tail if necessary. Return false if entries cannot be acquired.
   * 
   * @param arity
   *          The arity of this buffer (used for sanity test only).
   * @return True if there the head buffer has been successfully replenished.
   */
