...
/**
 * This class implements a local (<i>unsynchronized</i>) sequential store
 * buffer. An SSB is strictly FIFO (although this class does not implement
 * dequeuing).
 * <p>
 * 
 * Each instance stores word-sized values into a local buffer. When the buffer
 * is full, or if the <code>flushLocal()</code> method is called, the buffer
 * enqueued at the tail of a <code>SharedDeque</code>. This class provides no
 * mechanism for dequeing.
 * <p>
 * 
 * The implementation is intended to be as efficient as possible, in time and
 * space, as it is used in critical code such as the GC work queue and the write
 * buffer used by many "remembering" collectors. Each instance has just two
 * fields: a bump pointer and a pointer to the <code>SharedDeque</code>
 * <p>
 * 
 * Preconditions: Buffers are always aligned on buffer-size address boundaries.
 * <p>
 * 
 * Invariants: Buffers are filled such that tuples (of the specified arity) are
 * packed to the low end of the buffer. Thus buffer overflows on inserts and
 * pops (underflow actually) will always arise when then cursor is buffer-size
 * aligned.
 * 
...
class LocalSSB extends Deque implements Constants, Uninterruptible {
   ...
   * 
   * @param queue
   *          The shared queue to which this local ssb will append its buffers
  ...
  /**
   * Flush the buffer and add it to the shared queue (this will make any entries
   * in the buffer visible to any consumer associated with the shared queue).
   */
   ...
   * to the shared queue if not null).
   * 
   * @param arity
   *          The arity of the values stored in this SSB: the buffer must
  ...
  protected final void checkTailInsert(int arity) throws InlinePragma {
    if (bufferOffset(tail).isZero())
      ...
      Assert._assert(bufferOffset(tail).sGE(
          Word.fromIntZeroExtend(arity).lsh(LOG_BYTES_IN_ADDRESS).toOffset()));
  }

  /**
   * Insert a value into the buffer. This is <i>unchecked</i>. The caller must
   * first call <code>checkInsert()</code> to ensure the buffer can
  ...
  protected final void uncheckedTailInsert(Address value) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(bufferOffset(tail).sGE(
          Offset.fromIntZeroExtend(BYTES_IN_ADDRESS)));
  ...
  /**
   * In the case where a buffer must be flushed before being filled (either to
   * the queue or to the head), the entries must be slid to the base of the
   * buffer in order to preserve the invariant that all non-tail buffers will
   * have entries starting at the base (which allows a simple test against the
   * base to be used when popping entries). This is <i>expensive</i>, so should
   * be avoided.
   * 
   * @param arity
   *          The arity of the buffer in question
  ...
  /**
   * Return the sentinel offset for a buffer of a given arity. This is used both
   * to compute the address at the end of the buffer.
   * 
   * @param arity
  ...
  /**
   * Close the tail buffer (normalizing if necessary), and enqueue it at the
   * tail of the shared buffer queue.
   * 
   * @param arity
   *          The arity of this buffer.
   */
