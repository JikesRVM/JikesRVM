 ...
 * deque. This class is based upon the SharedQueue class. The sorting routines
 * were modified from code written by Narendran Sachindran and Matthew Hertz for
 * GCTk.
 * 
 ...
 * @date $Date$
 */
public abstract class SortSharedDeque extends SharedDeque implements
    Uninterruptible {
   ...
   * 
   * @param rps
   *          The space from which the instance should obtain buffers.
   */
  ...
  /**
   * Find the highest bit that is set in a longword and return a mask with only
   * that bit set.
   * 
   * @param addr
   ...
   * 
   * @param begin
   *          Start address of the range to be sorted
   * @param end
   *          End address of the range to be sorted
   */
  ...
  /**
   * Sort objects using radix exchange sort. An explicit stack is maintained to
  ...
  public void sort() {
    ...
    if (!head.EQ(HEAD_INITIAL_VALUE)) {
      ...
      /*
       * Obtain the bitmask for the first iteration and save the start & end
       * pointers and the bitmask on the stack
       */
      initStack();
      startPtr = popStack();
      while (!startPtr.isZero()) {
        ...
        if (startLink.NE(endLink)) {
          ...
          /*
           * Use insertionSort for a limited number of objects within a single
           * block
           */
  ...
  /**
   * Partition the slots in an address range based on the value in a particular
   * bit. Place the start & end addresses of the two partitions & a bitmask per
   * partition (which indicates the highest bit position at which the max & min
   * of a partition differ) on the stack. This works just like the partition in
   * quick sort, except that a bit value is being compared here
   * 
   * @param startAddr
   *          The start address of the range to be sorted
   * @param startLinkAddr
   *          The address where the start block has its next field
   * @param endAddr
   *          The end address of the range to be sorted
   * @param endLinkAddr
   *          The address where the end block has its next field
   * @param bitMask
   *          The mask in which the bit to be commpared is set
   */
  private final void partition(Address startAddr, Address startLinkAddr,
      Address endAddr, Address endLinkAddr, Word bitMask) {
    ...
    while (true) {
      ...
      /*
       * Move the left pointer until the right pointer is reached or an address
       * with a 0 value in the bit position is found.
       */
      while (true) {
      ...
      /*
       * Move the right pointer until the left pointer is reached or an address
       * with a 1 value in the bit position is found.
       */
      while (true) {
    ...
    /*
     * If max value is not equal to the min value in the right partition, (not
     * all slots are identical) push the right partition on to the stack
     */
    if (rmax.GT(rmin)) {
    ...
    /*
     * if max value is not equal to the min value in the left partition, (not
     * all slots are identical) push the left partition on to the stack
     */
    if (lmax.GT(lmin)) {
  ...
  /*
   * Allocate memory for the stack and intialize it with the first range to
   * partition
   */
