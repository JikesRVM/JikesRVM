 ...
 * @date $Date$
 */
public class TraceBuffer extends LocalQueue implements Constants,
    TracingConstants, Uninterruptible {
   ...
   * 
   * @param i
   *          The data to be pushed onto the tracing queue
   */
    ...
    while (checkDequeue(1)) {
      ...
      while (!bufferOffset(head).isZero()) {
        ...
        if (traceState.EQ(TRACE_NEW_RECORD)) {
          ...
          if (traceState.EQ(TRACE_EXACT_ALLOC) || traceState.EQ(TRACE_ALLOC)) {
