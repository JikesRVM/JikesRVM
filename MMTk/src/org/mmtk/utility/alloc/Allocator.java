 ...
 * The key functionality provided is the retry mechanism that is necessary to
 * correctly handle the fact that a "slow-path" allocation can cause a GC which
 * violate the uninterruptability assumption. This results in the thread being
 * moved to a different processor so that the allocator object it is using is
 * not actually the one for the processor it is running on.
 * 
 * This class also includes functionality to assist allocators with ensuring
 * that requests are aligned according to requests.
 * 
 * Failing to handle this properly will lead to very hard to trace bugs where
 * the allocation that caused a GC or allocations immediately following GC are
 * run incorrectly.
 * 
 * @author Perry Cheng
...
public abstract class Allocator implements Constants, Uninterruptible {
  ...
  /**
   * Aligns up an allocation request. The allocation request accepts a region,
   * that must be at least particle aligned, an alignment request (some power of
   * two number of particles) and an offset (a number of particles). There is
   * also a knownAlignment parameter to allow a more optimised check when the
   * particular allocator in use always aligns at a coarser grain than
   * individual particles, such as some free lists.
   * 
   * @param region
   *          The region to align up.
   * @param alignment
   *          The requested alignment
   * @param offset
   *          The offset from the alignment
   * @param knownAlignment
  ...
  final public static Address alignAllocation(Address region, int alignment,
      int offset, int knownAlignment, boolean fillAlignmentGap)
      throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS) {
      ...
      Assert._assert(region.toWord().and(
          Word.fromIntSignExtend(MIN_ALIGNMENT - 1)).isZero());
   ...
   * @param end
   *          A pointer past the end of the region.
   */
   ...
   * Aligns up an allocation request. The allocation request accepts a region,
   * that must be at least particle aligned, an alignment request (some power of
   * two number of particles) and an offset (a number of particles).
   * 
   * @param region
   *          The region to align up.
   * @param alignment
   ...
   */
  final public static Address alignAllocation(Address region, int alignment,
      int offset) throws InlinePragma {
   ...
   * @return The aligned up address.
   */
  final public static Address alignAllocationNoFill(Address region,
      int alignment, int offset) throws InlinePragma {
   ...
   * 
   * @param size
   *          The number of bytes (not aligned).
   * @param alignment
   *          The requested alignment (some factor of 2).
   */
   ...
   *          The requested alignment (some factor of 2).
   * @param knownAlignment
   *          The known minimum alignment. Specifically for use in allocators
   *          that enforce greater than particle alignment.
   */
  final public static int getMaximumAlignedSize(int size, int alignment,
      int knownAlignment) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(knownAlignment >= MIN_ALIGNMENT);
  ...
  public Address allocSlow(int bytes, int alignment, int offset, boolean inGC)
      throws NoInlinePragma {
  ...
  private Address allocSlowBody(int bytes, int alignment, int offset,
    ...
    Log
        .write("GC Warning: Possible VM range imbalance - Allocator.allocSlowBody failed on request of ");
    Log.write(bytes);
    Log.write(" on space ");
