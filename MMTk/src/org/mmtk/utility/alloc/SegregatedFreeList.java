 ...
 * <p>
 * 
 * See: Wilson, Johnstone, Neely and Boles "Dynamic Storage Allocation: A Survey
 * and Critical Review", IWMM 1995, for an overview of free list allocation and
 * the various implementation strategies, including segregated free lists.
 * <p>
 * 
 * We maintain a number of size classes, each size class having a free list of
 * available objects of that size (the list may be empty). We call the storage
 * elements "cells". Cells reside within chunks of memory called "blocks". All
 * cells in a given block are of the same size (i.e. blocks are homogeneous with
 * respect to size class). Each block maintains its own free list (free cells
 * within that block). For each size class a list of blocks is maintained, one
 * of which will serve the role of the current free list. When the free list on
 * the current block is exhausted, the next block for that size class becomes
 * the current block and its free list is used. If there are no more blocks the
 ...
 * @date $Date$
 */
public abstract class SegregatedFreeList extends Allocator implements
  ...
  private static final Word WORD_SHIFT_MASK = Word.one().lsh(LOG_BITS_IN_WORD)
      .minus(Extent.one());
  ...
  public static final int META_DATA_PAGES_PER_REGION = Conversions
   ...
   * 
   * @param space
   *          The space with which this allocator will be associated
   */
  ...
  /**
   * Allocate <code>bytes</code> contigious bytes of zeroed memory.
   * <p>
   * 
   * This code first tries the fast version and, if needed, the slow path.
   * 
   * @param bytes
   *          The size of the object to occupy this space, in bytes.
   * @param align
   *          The requested alignment.
   * @param offset
   *          The alignment offset.
   * @param inGC
   *          If true, this allocation is occuring with respect to a space that
   *          is currently being collected.
   * @return The address of the first word of <code>bytes</code> contigious
   *         bytes of zeroed memory.
   */
   ...
   * <p>
   * 
   * This code must be efficient and must compile easily. Here we minimize the
   * number of calls to inlined functions, and force the "slow path" (uncommon
   * case) out of line to reduce pressure on the compiler. We have a call to an
   * abstract method that allows subclasses to customize post-allocation.
   * 
   * @param bytes
   *          The size of the object to occupy this space, in bytes.
   * @param align
   ...
   *         bytes of zeroed memory.
   */
  public final Address allocFast(int bytes, int align, int offset, boolean inGC)
      throws InlinePragma {
  ...
  /**
   * Allocate <code>bytes</code> contigious bytes of non-zeroed memory. First
   * check if the fast path works. This is needed since this method may be
   * called in the context when the fast version was NOT just called. If this
   * fails, it will try finding another block with a non-empty free list, or
   * allocating a new block.
   * <p>
   * 
   * This code should be relatively infrequently executed, so it is forced out
   * of line to reduce pressure on the compilation of the core alloc routine.
   * <p>
   * 
   * Precondition: None
   * 
   * Postconditions: A new cell has been allocated (not zeroed), and the block
   * containing the cell has been placed on the appropriate free list data
   * structures. The free list itself is not updated (the caller must do so).
   * <p>
   * 
   * @param bytes
   *          The size of the object to occupy this space, in bytes.
  ...
  /**
   * Expand a particular size class, allocating a new block, breaking the block
   * into cells and placing those cells on a free list for that block. The block
   * becomes the current head for this size class and the address of the first
   * available cell is returned.
   * <p>
   * 
   ...
   * @return The address of the first available cell in the newly allocated
   *         block of pre-zeroed cells, or return zero if there were
   *         insufficient resources to allocate a new block.
   */
  private final Address expandSizeClass(int sizeClass) throws InlinePragma {
   ...
   * 
   * @param cell
   *          The address of teh current cell.
   * @return The next cell in a free list chain (null if this is the last).
   */
  protected final Address getNextCell(Address cell) throws InlinePragma {
   ...
   * 
   * @param cell
   *          The cell whose link is to be set
   * @param next
  ...
  /**
   * Free a cell. The cell is added to the free list for the given block, and
   * the inuse count for the block is decremented, the block freed if empty.
   * <b>The cell is zeroed as it is freed.</b>
   * 
   * @param cell
   *          The cell to be freed
   * @param block
   *          The block on which the cell resides
   * @param sizeClass
   *          The size class of the cell and block
   * @param nextFree
   *          The next cell in the free list
   */
  public final void free(Address cell, Address block, int sizeClass,
      Address nextFree) throws InlinePragma {
  ...
  /**
   * Install a new block. The block needs to be added to the size class's linked
   * list of blocks and made the current block.
   * 
   * @param block
   *          The block to be added
   * @param sizeClass
   *          The size class to which the block is being added
   */
  ...
  /**
   * Free a block. The block needs to be removed from its size class's linked
   * list before being freed.
   * 
   * @param block
   *          The block to be freed
   * @param sizeClass
   *          The size class with which the block was associated.
   */
   ...
   * target of 1/8. In fact, across sizes from 8 bytes to 512 the average worst
   * case loss is 13.3%, giving an expected loss (assuming uniform distribution)
   * of about 7%. We avoid using the Lea class sizes because they were so
   * numerous and therefore liable to lead to excessive inter-class-size
   * fragmentation.
   * <p>
   * 
   ...
   *          allocated.
   * @return The size class capable of accomodating the allocation request. If
   *         the request is sufficiently large then
   ...
   */
  protected static final int getSizeClass(int bytes) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
  ...
  /**
   * Return the size of a basic cell (i.e. not including any cell header) for a
   * given size class.
   * 
   * @param sc
   ...
   */
  protected static final int getBaseCellSize(int sc) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
    ...
    if (BYTES_IN_ADDRESS == 32) { // 32-bit
      if (COMPACT_SIZE_CLASSES)
        return ((sc < 8) ? (sc + 1) << 2 : (sc < 12) ? (sc - 3) << 3
            : (sc < 16) ? (sc - 7) << 4 : (sc < 18) ? (sc - 13) << 6
                : (sc < 21) ? (sc - 16) << 8 : (sc - 19) << 10);
      else
        return ((sc < 16) ? (sc + 1) << 2 : (sc < 20) ? (sc - 11) << 4
            : (sc < 24) ? (sc - 15) << 5 : (sc < 28) ? (sc - 19) << 6
                : (sc < 34) ? (sc - 25) << 8 : (sc - 31) << 10);
    } else { // 64-bit
      if (COMPACT_SIZE_CLASSES)
        return ((sc < 12) ? (sc + 1) << 3 : (sc < 14) ? (sc - 5) << 4
            : (sc < 16) ? (sc - 9) << 5 : (sc < 19) ? (sc - 12) << 6
                : (sc < 20) ? (sc - 15) << 7 : (sc < 21) ? (sc - 18) << 9
                    : (sc - 19) << 10);
      else
        return ((sc < 14) ? (sc + 1) << 3 : (sc < 21) ? (sc - 6) << 4
            : (sc < 24) ? (sc - 13) << 5 : (sc < 28) ? (sc - 18) << 6
                : (sc < 34) ? (sc - 25) << 8 : (sc - 31) << 10);
    }
  }
   ...
   * <code>currentBlock</code> values, so instead of the free list pointing to
   * free cells, it points to the block containing the free cells. Then the free
   * lists for each cell can be reestablished during GC. If the free lists are
   * being preserved on a per-block basis (eager mark-sweep and reference
   * counting), then free lists are remembered for each block.
   */
  ...
  /**
   * Retrieve free list pointers from the first blocks in the free list.
   */
  ...
  /**
   * In the case where free lists associated with each block are preserved, get
   * the free list for a given block.
   * 
   * @param block
   *          The block whose free list is to be found
   * @return The free list for this block
   */
  protected final Address getFreeList(Address block) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
  ...
  /**
   * In the case where free lists associated with each block are preserved, set
   * the free list for a given block.
   * 
   * @param block
   *          The block whose free list is to be found
   * @param cell
   *          The head of the free list (i.e. the first cell in the free list).
   */
   ...
   * 
   * FIXME This is currently implemented by searching each *mutator* free list.
   * It needs to be per-collector, not per-mutator.
   */
  protected final void sweepBlocks() {
    for (int sizeClass = 0; sizeClass < SIZE_CLASSES; sizeClass++) {
      ...
      while (!block.isZero()) {
        /*
         * first check to see if block is completely free and if possible free
         * the entire block
         */
        Address next = BlockAllocator.getNextBlock(block);
      ...
      if (SORT_FREE_BLOCKS)
        reestablishBlockFreeList(sizeClass);
    }
  }

  /**
   * Add a block to a liveness bucket according to the specified liveness. This
   * allows a cheap approximation to sorting the blocks by liveness.
   * 
   * @param block
   *          the block to be added to a bucket
   * @param liveness
   *          the liveness of the block that is to be added
   */
  private final void addToBlockBucket(Address block, int liveness) {
    ...
    if (!tail.isZero())
      BlockAllocator.setNextBlock(tail, block);
  ...
  /**
   * Reestablish a free block list based on the ordering of buckets, taking the
   * lists of blocks from the buckets and composing them into a new free list of
   * blocks for a given sizeclass. The new free list is built up in LIFO (stack)
   * orderr, starting with the blocks that will be used last, and finishing with
   * the blocks that should be used first by the allocator.
   * 
   * @param sizeClass
   *          The sizeclass whose free block list is being composed
   */
  private final void reestablishBlockFreeList(int sizeClass) {
    ...
    if (!head.isZero())
      BlockAllocator.setPrevBlock(head, Address.zero());
    firstBlock.set(sizeClass, head);
  }

  /**
   * Add a bucket full of blocks to the front of the free block list for a given
   * class. This allows the LIFO (stack order) construction of new free lists
   * using buckets.
   * 
   * @param sizeClass
   *          The size class whose frelist is being built
   * @param head
   *          The current head of the free block list for this sizeclass
   * @param bucket
   *          The index of the bucket to be added to the front of this free
   *          block list.
   */
   ...
   * primitives---must only be used when contention for live bit is strictly not
   * possible
   * 
   * @param object
   *          The object whose live bit is to be set.
   ...
   */
  protected static final void deadAddress(Address address) throws InlinePragma {
    Word oldValue, newValue;
    Address liveWord = getLiveWordAddress(address);
  ...
  public static final void zeroLiveBits(Address start, Address end) {
    ...
    while (start.LT(end)) {
      Address metadata = EmbeddedMetaData.getMetaDataBase(start).plus(
          SegregatedFreeList.META_DATA_OFFSET);
  ...
  /**
   * Walk through a set of live bitmaps for a block, checking which are alive.
   * If <code>SORT_FREE_BLOCKS</code>and if all cells are unused, return
   * true.
   * 
   * @param block
   *          The block
   ...
   * @param count
   *          If true return a count of all non zero words, otherwise just
   *          return 1 if any live word exists
   * @return If <code>count</code> is true, return a count of all non-zero
   *         words, otherwise return 1 if any live word exists, zero otherwise.
   */
  ...
  /**
   * Use the live bits for a block to infer free cells and thus construct a free
   * list for the block.
   * 
   * @param block
   ...
   * 
   * @param address
   *          The address for which the live word is required
   ...
   * @param set
   *          True if we want the mask for <i>setting</i> the bit, false if we
  ...
  protected static final Word getMask(Address address, boolean set)
      throws InlinePragma {
    int shift = address.toWord().rshl(OBJECT_LIVE_SHIFT).and(WORD_SHIFT_MASK)
        .toInt();
