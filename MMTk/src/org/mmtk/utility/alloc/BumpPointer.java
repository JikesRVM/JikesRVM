...
/**
 * This class implements a bump pointer allocator that allows linearly scanning
 * through the allocated objects. In order to achieve this in the face of
 * parallelism it maintains a header at a region (1 or more chunks) granularity.
 * 
 * Intra-block allocation is fast, requiring only a load, addition comparison
 * and store. If a block boundary is encountered the allocator will request more
 * memory (virtual and actual).
 * 
 * In the current implementation the scanned objects maintain affinity with the
 * thread that allocated the objects in the region. In the future it is
 * anticipated that subclasses should be allowed to choose to improve load
 * balancing during the parallel scan.
 * 
 * Each region is laid out as follows:
 * 
 * +-------------+-------------+-------------+--------------- | Region End |
 * Next Region | Data End | Data -->
 * +-------------+-------------+-------------+---------------
 * 
 * The minimum region size is 32768 bytes, so the 3 or 4 word overhead is less
 * than 0.05% of all space.
 * 
 * An intended enhancement is to facilitate a reallocation operation where a
 * second cursor is maintained over earlier regions (and at the limit a lower
 * location in the same region). This would be accompianied with an alternative
 * slow path that would allow reuse of empty regions.
 * 
 ...
 * @date $Date$
 */
public class BumpPointer extends Allocator implements Constants,
    Uninterruptible {
  ...
  protected static final Word CHUNK_MASK = Word.one().lsh(LOG_CHUNK_SIZE)
      .minus(Word.one());

  // Offsets into header
  protected static final Offset REGION_LIMIT_OFFSET = Offset.zero();

  protected static final Offset NEXT_REGION_OFFSET = REGION_LIMIT_OFFSET
      .plus(BYTES_IN_ADDRESS);

  protected static final Offset DATA_END_OFFSET = NEXT_REGION_OFFSET
      .plus(BYTES_IN_ADDRESS);
  ...
  protected final boolean allowScanning; // linear scanning is permitted if
                                          // true
   ...
   * @param allowScanning
   *          Allow linear scanning of this region of memory.
   */
  ...
  /**
   * Reset the allocator. Note that this does not reset the space. This is must
  ...
  /**
   * Re-associate this bump pointer with a different space. Also reset the bump
   * pointer so that it will use the new space on the next call to
   * <code>alloc</code>.
   * 
   * @param space
   *          The space to associate the bump pointer with.
   */
  ...
  /**
   * Allocate space for a new object. This is frequently executed code and the
   * coding is deliberaetly sensitive to the optimizing compiler. After changing
   * this, always check the IR/MC that is generated.
   * 
   * @param bytes
  ...
  /**
   * Allocation slow path (called by superclass when slow path is actually
   * taken. This is necessary (rather than a direct call from the fast path)
   * because of the possibility of a thread switch and corresponding
   * re-association of bump pointers to kernel threads.
   * 
   * @param bytes
   *          The number of bytes allocated
   * @param align
   ...
   *          The offset from the alignment
   * @param inGC
   *          Was the request made from within GC?
   * @return The address of the first byte of the allocated region or zero on
   *         failure
   */
  final protected Address allocSlowOnce(int bytes, int align, int offset,
    ...
    if (allowScanning && !region.isZero()) {
      ...
      if (!nextRegion.isZero()) {
        ...
        Memory.zero(cursor, limit.diff(cursor).toWord().toExtent().plus(
            BYTES_IN_ADDRESS));

        ((MarkCompactSpace) space).reusePages(Conversions.bytesToPages(limit
            .diff(region).plus(BYTES_IN_ADDRESS)));
    ...
    Extent chunkSize = Word.fromIntZeroExtend(bytes).plus(CHUNK_MASK).and(
        CHUNK_MASK.not()).toExtent();
   ...
   * @param size
   *          The size of the new region (rounded up to chunk-alignment)
   */
  private void updateMetaData(Address start, Extent size) throws InlinePragma {
    if (initialRegion.isZero()) {
      ...
      cursor = region.plus(DATA_START_OFFSET);
        ...
        || region.diff(start.plus(size)).toWord().toExtent().GT(
            maximumRegionSize())) {
   ...
   * 
   * @param scanner
   *          The scan object to delegate scanning to.
   */
  public void linearScan(LinearScan scanner) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(allowScanning);
   ...
   * 
   * @param scanner
   *          The scan object to delegate to.
   * @param start
  ...
  private void scanRegion(LinearScan scanner, Address start)
    ...
    ObjectReference current = ObjectModel.getObjectFromStartAddress(start
        .plus(DATA_START_OFFSET));
  ...
  /**
   * Maximum size of a single region. Important for children that implement load
   * balancing or increments based on region size.
   * 
