 ...
 * <p>
 * 
 * Blocks are a non-shared (thread-local) coarse-grained unit of storage. Blocks
 * are available in power-of-two sizes.
 * 
 * Virtual memory space is taken from a VM resource, and pages consumed by
 * blocks are accounted for by a memory resource.
 * 
...
public final class BlockAllocator implements Constants, Uninterruptible {
  ...
  public static final int LOG_BLOCKS_IN_REGION = EmbeddedMetaData.LOG_BYTES_IN_REGION
      - LOG_MIN_BLOCK;

  private static final int SUB_PAGE_SIZE_CLASS = LOG_BYTES_IN_PAGE
      - LOG_MIN_BLOCK - 1;
  ...
  private static final byte MAX_BLOCK_SIZE_CLASS = LOG_MAX_BLOCK
      - LOG_MIN_BLOCK;
  ...
  private static final int LOG_BYTE_COVERAGE = LOG_MIN_BLOCK
  ...
  public static final Extent META_DATA_EXTENT = Extent
      .fromIntSignExtend(META_DATA_BYTES_PER_REGION);
  ...
  /**
   * Allocate a block, returning the address of the first usable byte in the
   * block.
   * 
   * @param blockSizeClass
   *          The size class for the block to be allocated.
   * @return The address of the first usable byte in the block, or zero on
   *         failure.
   */
  ...
  /**
   * Free a block. If the block is a sub-page block and the page is not
   * completely free, then the block is added to the free list. Otherwise the
   * block is returned to the virtual memory resource.
   * 
   * @param block
  ...
  final void free(Address block) {
    ...
    if (blockSizeClass <= SUB_PAGE_SIZE_CLASS) {
      ...
      if (decInUseCount(block) == 0) {
        ...
        space.release(block);
        if (PARANOID)
          freeBlocks[blockSizeClass] -= BYTES_IN_PAGE
      ...
      // whole page or pages, so simply return it to the resource
      space.release(block);
    if (PARANOID)
      sanity(false);
  }
  ...
  /**
   * Take a block off the free list (if available), updating the free list and
   * inuse counters as necessary. Return the first usable byte of the block, or
   * zero on failure.
   * 
   * @param blockSizeClass
   *          The size class for the block to be allocated.
   ...
   *         failure.
   */
  private final Address allocFast(int blockSizeClass) throws InlinePragma {
    Address rtn;
  ...
  /**
   * Acquire new pages (if possible), and carve them up for use as blocks.
   * Return the address of the first usable byte of the first block on success,
   * or zero on failure. If successful, any subblocks will be added to the free
   * list.
   * 
   * @param blockSizeClass
   *          The size class for the block to be allocated.
  ...
  /**
   * Given a page of sub-page blocks, initialize <code>inUseCount</code> and
   * <code>blockSizeClass</code> for that page and put all blocks except the
   * first on the free list (the first will implicitly be used immediately).
   * 
   * @param start
   *          The start of the page
   * @param blockSizeClass
   *          The sizeClass of the blocks to go in this page.
   */
  ...
  /**
   * Return the number of pages required when allocating space for this size
   * class.
   * 
   * @param blockSizeClass
   *          The size class in question
   * @return The number of pages required when allocating a block (or blocks) of
   *         this size class.
   */
   ...
   * 
   * @param block
   *          One of the one or more blocks in the set whose count is to be
   *          reset to 1.
   */
  private static void resetInUseCount(Address block) throws InlinePragma {
   ...
   * 
   * @param block
   *          One of the blocks in the set whose count is being incremented.
   */
  ...
  /**
   * Decrement the inuse count for a set of blocks and return the post-decrement
   * value. This says how many blocks are in use on a given page.
   * 
   * @param block
   *          One of one or more blocks in the set whose count is being
   *          decremented.
  ...
  /**
   * Set the <i>in use</i> meta data field for a given page. This says how many
   * blocks are in use on a given page.
   * 
   * @param address
   *          The address of interest
   * @param iu
   *          The value to which this field is to be set
   */
  ...
  /**
   * Get the block's <i>in use</i> meta data field for a given page. This says
   * how many blocks are in use on a given page.
   * 
   * @param address
   *          The address of interest
   * @return The inuse field for the block containing the given address
   */
  private static final short getInUseCount(Address address) throws InlinePragma {
    address = Conversions.pageAlign(address);
    return getMetaAddress(address).loadShort(IU_OFFSET);
  }

  /**
   * Set the <i>block size class</i> meta data field for a given address (all
   * blocks on a given page are homogeneous with respect to block size class).
   * 
   * @param address
   *          The address of interest
   * @param sc
  ...
  /**
   * Get the <i>block size class</i> meta data field for a given page (all
   * blocks on a given page are homogeneous with respect to block size class).
   * 
   * @param address
   *          The address of interest
  ...
  /**
   * Set the free list meta data field for a given address (this is per-block
   * meta data that is stored along with the block metadata but not used by the
   * block allocator).
   * 
   * @param address
   *          The address of interest
   ...
   *          The value to which this field is to be set
   */
  public static final void setFreeListMeta(Address address, Address value)
      throws InlinePragma {
    getMetaAddress(address).plus(FL_META_OFFSET).store(value);
  }

  /**
   * Get the free list meta data field for a given address (this is per-block
   * meta data that is stored along with the block metadata but not used by the
   * block allocator).
   * 
   * @param address
   *          The address of interest
   * @return The free list meta data field for the block containing the given
   *         address
   */
   ...
   * 
   * @param block
   *          The block to be removed from the doubly linked list
   */
  static final void unlinkBlock(Address block) throws InlinePragma {
    ...
    // }
    if (!prev.isZero())
      setNextBlock(prev, next);
    if (!next.isZero())
      setPrevBlock(next, prev);
  }
   ...
   * @param prev
   *          The block that is to preceed the new block
   */
   ...
   */
  static final Address getPrevBlock(Address address) throws InlinePragma {
   ...
   */
  public static final Address getNextBlock(Address address) throws InlinePragma {
    return getMetaAddress(address, NEXT_OFFSET).loadAddress();
  }

  /**
   * Get the address of some metadata given the address for which the metadata
   * is required and the offset into the metadata that is of interest.
   * 
   * @param address
   *          The address for which the metadata is required
   ...
   *          The address for which the metadata is required
   * @param offset
   *          The offset (in bytes) into the metadata block (eg for the prev
   *          pointer, or next pointer)
   * @return The address of the specified meta data
   */
  private static final Address getMetaAddress(Address address, Offset offset)
      throws InlinePragma {
    return EmbeddedMetaData.getMetaDataBase(address).plus(
        EmbeddedMetaData.getMetaDataOffset(address, LOG_BYTE_COVERAGE,
            LOG_BYTES_IN_BLOCK_META)).plus(offset);
  }
  ...
  /**
   * Given some block, unlink all blocks on the page in which that block resides
   * and return the address of the containing page.
   * 
   * @param block
   *          The block whose page is to be unlinked
   * @param blockSizeClass
   *          The size class of the page
  ...
  /**
   * Perform a basic sanity test, checking the contents of each of the block
   * free lists.
   * 
   * @param verbose
   *          If true then produce large amounts of debugging output detailing
   *          the composition of the free lists.
   */
  ...
  /**
   * Perform a sanity traversal of a linked list of blocks, checking that the
   * double links are sane, and returning the length of the list.
   * 
   * @param block
   *          The first block in the list to be checked
   * @param prev
   *          The previous block in the list (possibly null)
   * @param verbose
   *          If true then produce large amounts of debugging output detailing
