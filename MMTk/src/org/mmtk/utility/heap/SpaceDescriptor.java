 ...
 * <p>
 * 
 * Space descriptors are integers that encode a space's mapping into virtual
 * memory. For discontigious spaces, they indicate discontiguity and mapping
 * must be done by consulting the space map. For contigious spaces, the space's
 * address range is encoded into the integer (using a fixed point notation).
 * <p>
 * 
 * The purpose of this class is to allow <code>static final int</code> space
 * descriptors to exist for each space, which can then be used in tests to
 * determine whether an object is in a space. A good compiler can perform this
 * decoding at compile time and produce optimal code for the test.
 * 
 * $Id$
 * 
...
public class SpaceDescriptor implements Uninterruptible, Constants {
  ...
  private static final int VM_MANTISSA_SHIFT = VM_EXPONENT_SHIFT
      + VM_EXPONENT_BITS;
   ...
   * @return An integer descriptor encoding the region of virtual memory
   *         occupied by the space
   */
  public static int createDescriptor(Address start, Address end) {
    int chunks = end.diff(start).toWord().rshl(Space.LOG_BYTES_IN_CHUNK)
        .toInt();
    ...
    return (mantissa << VM_MANTISSA_SHIFT) | (exponent << VM_EXPONENT_SHIFT)
   ...
   * 
   * @return An integer descriptor reflecting the fact that this space is shared
   *         (and thus discontigious and so must be established via maps).
   */
  ...
  /**
   * Return true if this descriptor describes a contigious space that is at the
  ...
  public static Address getStart(int descriptor) throws InlinePragma {
    if (Assert.VERIFY_ASSERTIONS)
      Assert._assert(isContiguous(descriptor));
  ...
  /**
   * Return the size of the region of memory encoded in this descriptor, in
   * chunks
   * 
   * @param descriptor
   * @return The size of the region of memory encoded in this descriptor, in
