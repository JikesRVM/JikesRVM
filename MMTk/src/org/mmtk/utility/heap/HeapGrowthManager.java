...
/**
 * This class is responsible for growing and shrinking the heap size by
 * observing heap utilization and GC load.
 * 
 * @author Perry Cheng
 * @author Dave Grove
 */
public abstract class HeapGrowthManager implements Constants, Uninterruptible {
  ...
  private final static double[][] generationalFunction = {
  ...
  private final static double[][] nongenerationalFunction = {
      { 0.00, 0.00, 0.10, 0.30, 0.60, 0.80, 1.00 },
      { 0.00, 0.90, 0.90, 0.95, 1.00, 1.00, 1.00 },
  ...
  /**
   * An encoding of the function used to manage heap size. The xaxis represents
   * the live ratio at the end of a major collection. The yaxis represents the
   * GC load (GC time/total time). The interior of the matrix represents a ratio
   * to shrink or grow the heap for a given pair of live ratio and GC load. The
   * constraints on the matrix are:
   * <ul>
   * <li> function[0][0] is ignored.
   * <li> All numbers in the first row must monotonically increase and must be
   * in the range from 0 to 1 inclusive.</li>
   * <li> All numbers in the first column must monotonically increase and must
   * be in the range from 0 to 1 inclusive.</li>
   * <li> There must be 0 and 1 values specified in both dimensions.
   * <li> For all interior points in the matrix, the value must be greater than
   * the liveRatio for that column.</li>
   * </ul>
   */
  private static final double[][] function = ActivePlan.constraints()
      .generational() ? generationalFunction : nongenerationalFunction;
  ...
  /**
   * Initialize heap size parameters and the mechanisms used to adaptively
   * change heap size.
   */
  ...
  /**
   * Forcibly grow the heap by the given number of bytes. Used to provide
   * headroom when handling an OutOfMemory situation.
   * 
   * @param size
   *          number of bytes to grow the heap
   */
  ...
  /**
   * Record the time taken by the current GC; used to compute gc load, one of
   * the inputs into the heap size management function
   */
  ...
  /**
   * Decide how to grow/shrink the heap to respond to application's memory
   * usage.
   * 
  ...
  public static boolean considerHeapSize() {
    ...
    Extent newSize = Word.fromIntSignExtend(
        (int) (ratio * (double) (oldSize.toLong() >> LOG_BYTES_IN_MBYTE))).lsh(
        LOG_BYTES_IN_MBYTE).toExtent(); // do arith in MB to avoid overflow
    if (newSize.LT(reserved))
      newSize = reserved;
    newSize = newSize.plus(BYTES_IN_MBYTE - 1).toWord()
        .rshl(LOG_BYTES_IN_MBYTE).lsh(LOG_BYTES_IN_MBYTE).toExtent(); // round
                                                                      // to next
                                                                      // megabyte
    if (newSize.GT(maxHeapSize))
      newSize = maxHeapSize;
  ...
  private static double computeHeapChangeRatio(double liveRatio) {
    ...
    while (true) {
      if (function[0][liveRatioUnder + 1] >= liveRatio)
        break;
      liveRatioUnder++;
    }
    while (true) {
      if (function[0][liveRatioAbove - 1] <= liveRatio)
        break;
      liveRatioAbove--;
    }
    while (true) {
      if (function[gcLoadUnder + 1][0] >= gcLoad)
        break;
      gcLoadUnder++;
    }
    while (true) {
      if (function[gcLoadAbove - 1][0] <= gcLoad)
        break;
      gcLoadAbove--;
    }
    ...
    double liveRatioFraction = (liveRatio - function[0][liveRatioUnder])
        / (function[0][liveRatioAbove] - function[0][liveRatioUnder]);
    double liveRatioDelta = function[gcLoadUnder][liveRatioAbove]
        - function[gcLoadUnder][liveRatioUnder];
    factor += (liveRatioFraction * liveRatioDelta);
    double gcLoadFraction = (gcLoad - function[gcLoadUnder][0])
        / (function[gcLoadAbove][0] - function[gcLoadUnder][0]);
    double gcLoadDelta = function[gcLoadAbove][liveRatioUnder]
        - function[gcLoadUnder][liveRatioUnder];
