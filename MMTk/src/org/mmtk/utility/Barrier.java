...
/**
 * This class implements barrier synchronization. The mechanism handles proper
 * resetting by usnig 3 underlying counters and supports unconditional blocking
 * until the number of participants can be determined.
 * 
 * @author Perry Cheng
 */
public final class Barrier implements Uninterruptible {
  ...
  // The value of target is one more than the number of threads we expect to
  // arrive.
  ...
  // Debugging constants
  private static final int TIME_CHECK = 1000000; // Check time every
                                                  // TIME_CHECK-th iteration

  private static long WARN_PERIOD = Long.MAX_VALUE; // set to a real value by
                                                    // fullyBooted

  private static long TIME_OUT = Long.MAX_VALUE; // set to a real value by
                                                  // fullyBooted

  public static void fullyBooted() {
    WARN_PERIOD = Statistics.secsToCycles(3); // Print msg every WARN_PERIOD
                                              // seconds
  ...
  public int arrive(int where) {
    ...
    if (myValue + 2 == target) {
      ...
      counters[prev].reset(); // everyone has seen the value so safe to reset
                              // now
      if (next == 0)
        ...
        if (((i - 1) % TIME_CHECK) == 0) {
          if (startCheck == 0) {
            ...
            if (elapsed - lastElapsed > WARN_PERIOD) {
              Log.write("GC Warning: Barrier wait has reached ");
              Log.write(Statistics.cyclesToSecs(elapsed));
              Log.write(" seconds.  Called from ");
              Log.write(where);
              Log.write(".  myOrder = ");
              Log.write(myValue);
              Log.write("  count is ");
              Log.write(c.peek());
              Log.write(" waiting for ");
              Log.write(target - 1);
              Log.writeln();
              lastElapsed = elapsed;
            }
