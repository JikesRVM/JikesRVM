...
public class ReferenceGlue {
   ...
   * (rather than in a table, for example). In this context references are soft,
  ...
  /**
   * Scan through all references and forward. Only called when references are
   * objects.
   */
   ...
   * referent is no longer sufficiently reachable. The definition of "reachable"
   * is defined by the semantics of the particular subclass of Reference. The
   * implementation of this routine is determined by the the implementation of
   * java.lang.ref.ReferenceQueue in GNU classpath. It is in this class rather
   * than the public Reference class to ensure that Jikes has a safe way of
   * enqueueing the object, one that cannot be overridden by the application
   * program.
   * 
