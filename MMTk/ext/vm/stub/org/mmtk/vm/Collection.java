...
public class Collection {
   ...
   * available heap memory exceeds this percentage the memory manager will throw
   * an OutOfMemoryError.
   */
   ...
   * ensure a GC is not in the process of completing, to avoid, for example, an
   * async GC being triggered on the switch from GC to mutator thread before all
   * GC threads have switched.
   * 
  ...
  /**
   * Schedule the finalizerThread, if there are objects to be finalized and the
   * finalizerThread is on its queue (ie. currently idle). Should be called at
   * the end of GC after moveToFinalizable has been called, and before mutators
   * are allowed to run.
   */
