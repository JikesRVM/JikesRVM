...
public class Memory {
  ...
  /**
   * Allows for the VM to reserve space between HEAP_START() and
   * AVAILABLE_START() for its own purposes. MMTk should expect to encounter
   * objects in this range, but may not allocate in this range.
   * 
   * MMTk expects the virtual address space between AVAILABLE_START() and
   * AVAILABLE_END() to be contiguous and unmapped.
   * 
  ...
  /**
   * Allows for the VM to reserve space between HEAP_END() and AVAILABLE_END()
   * for its own purposes. MMTk should expect to encounter objects in this
   * range, but may not allocate in this range.
   * 
   * MMTk expects the virtual address space between AVAILABLE_START() and
   * AVAILABLE_END() to be contiguous and unmapped.
   * 
  ...
  /**
   * Return the space associated with/reserved for the VM. In the case of Jikes
   * RVM this is the boot image space.
   * <p>
   * 
  ...
  /**
   * Wait for preceeding cache flush/invalidate instructions to complete on all
   * processors. Ensures that all memory writes before this point are visible to
   * all processors.
   */
  ...
  /**
   * Wait for all preceeding instructions to complete and discard any prefetched
   * instructions on this processor. Also prevents the compiler from performing
   * code motion across this point.
   */
