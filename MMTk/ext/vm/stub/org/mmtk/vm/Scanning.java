...
public class Scanning implements Constants {
   ...
   * 
   * @param trace
   *          The trace object to use for precopying.
   * @param object
   *          The object to be scanned.
   */
  ...
  /**
   * Delegated enumeration of the pointers in an object, calling back to a given
   * plan for each pointer encountered.
   * 
   * @param object
   *          The object to be scanned.
   * @param e
   *          the Enumerator object through which the callback is made
   */
   ...
   * crucial that these instances are forwarded <i>prior</i> to the GC proper.
   * Since these instances <i>are not</i> enqueued for scanning, it is
   * important that when roots are computed the same instances are explicitly
   * scanned and included in the set of roots. The existence of this method
   * allows the actions of calculating roots and forwarding GC instances to be
   * decoupled.
   */
   ...
   * places them in the root values, root locations and interior root locations
   * queues. This method should not have side effects (such as copying or
   * forwarding of objects). There are a number of important preconditions:
   * 
   * <ul>
   * <li> All objects used in the course of GC (such as the GC thread objects)
   * need to be "pre-copied" prior to calling this method.
   * <li> The <code>threadCounter</code> must be reset so that load balancing
   * parallel GC can share the work of scanning threads.
   * </ul>
   * 
   * @param The
   *          trace to use for computing roots.
   */
