...
It is possible to use other 1.4 JVMs to build Jikes RVM;
however, building Jikes RVM is a good way to find bugs in a JVM.\@
Making a {\tt Full} build of Jikes RVM will make many JVMs exit abnormally.
...
You will need at least Jikes version 1.22; older versions of Jikes
will no longer compile recent versions of Jikes RVM and/or GNU
classpath. 

\item Download Jikes RVM from the \xlink{Jikes RVM
download page}{\RVMDownloadURL}, and unpack it.   You can either
  download a tarball via FTP or download directly from SVN.  We shall
...
person installing Jikes RVM. If you have any difficulty, look
\AtManualClasspathRootDiscussion{} 

\item Edit a configuration file so that it fits your machine.  One of
the {\tt i686-pc-linux} files in {\tt \$RVM\_\-ROOT/config} will probably be
...
file {\tt \$RVM\_\-ROOT/config/i686-pc-linux-gnu{\it .mine}}
...
{\tt \$RVM\_\-ROOT/bin} should be in your path:
...
\newcommand{\zipURL}{ftp://www.info-zip.org/Zip.html}
...
Jikes\TMweb{} Compiler & 1.22+ & \xlink{\tt \jikesURL}{\jikesURL}       \\
...
contents of this repository with SVN from the 
\xlink{public repository}{\RVMSVNURL}.
...
one working directory.   \texonly{See Section~\ref{configs}
(page~\pageref{configs}) for information about the various
configurations.} 

The Jikes RVM {\em boot image\index{boot image}} and other files
generated during the configuration process are stored in a {\em build
directory\index{build directory}}, which is logically separate from
the working directory.  Each distinct configuration of \jrvm{} you
build will have its own {\em build directory}.
...
You must edit a script in the \texttt{\$RVM\_\-ROOT/config}
directory to set  
...
environments in the \texttt{\$RVM\_\-ROOT/config} directory.  
...
You will use the \texttt{j\-con\-fi\-gure} script (in \texttt{\$RVM\_\-ROOT/bin}) to
...
\texttt{\$RVM\_ROOT/GNUmakefile}, you will need \texttt{xargs}
from GNU findutils.

\item
If you want to \texttt{make conditionals} using
\texttt{\$RVM\_ROOT/GNUmakefile}, you will need GNU sed.
...
in \texttt{\$RVM\_\-ROOT/doc}.  Additionally, the 
\xlink{developerWorks web page}{\RVMHomeURL} keeps an online version of
the user's guide and Javadoc\TMweb{} API, corresponding to the 
most recent release of Jikes RVM.
...
\texttt{\$RVM\_\-ROOT/doc/userguide} for rules on how to build the
...
Javadoc pages, use 
the \texttt{jdoc.sh} script in \texttt{\$RVM\_\-ROOT/bin}; this
