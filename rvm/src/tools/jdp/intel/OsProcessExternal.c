...
void lookup_Linux_signal(char *buf, int value);
  ...
  (JNIEnv *env, jobject obj, jint debuggee_pid) 
{
  ...
  printf("ERROR:  Platform.getSystemThreadId1 not implemented yet ...\n");
  return NULL;
...
/* it seems the call getthrds() is not available in Lintel */
int getSystemThreadId(int debuggee_pid, int **thread_id_array) {
  printf("ERROR: getSystemThreadId not available on Lintel\n");
  return 0;
}
...
JNIEXPORT jstring JNICALL Java_Platform_listSystemThreads1
  (JNIEnv *env, jobject obj, jint debuggee_pid)
{
  printf("ERROR: Platform.listSystemThreads1 not implemented yet ...\n");
  return NULL;
...
JNIEXPORT jintArray JNICALL Java_Platform_getVMThreadIDInSystemThread1
  (JNIEnv *env, jobject obj, jint debuggee_pid)
{
  printf("ERROR: Platform.getVMThreadIDInSystemThread1 not implemented yet ...\n");
  return NULL;
...
{
  printf("ERROR: mcontinueThread1 not implemented yet \n");
  return;
  ...
  printf("You have done a bad bad thingi (referring to uninitialized thread id)\n");
  ...
  (JNIEnv *env, jobject obj, jint debuggee_pid, jint waitstatus)
{
  ...
  rc = ptrace(PTRACE_CONT, debuggee_pid, (int *) 1, threadsig);
}
...
/**********************************************************
 * Single step using Lintel ptrace, return true if the debuggee
 * is still running, false if it has terminated
 * Class:     Platform
 * Method:    mstep1(I)Z
 */
JNIEXPORT jboolean JNICALL Java_Platform_mstep1
  (JNIEnv *env, jclass cls, jint debuggee_pid)
{
  int rc, status;
  pid_t wakeup_pid;

  /* printf("mstep1: single step process %d\n", debuggee_pid); */
  rc = ptrace(PTRACE_SINGLESTEP, debuggee_pid, 0, 0);
  if (rc!=0) {
    printf("mstep: hardware single step fails, errno = %d\n", errno);
    return JNI_TRUE;
  }
...
JNIEXPORT void JNICALL Java_Platform_mkill1
  (JNIEnv *env, jobject obj, jint debuggee_pid)
{
  int rc = ptrace(PTRACE_KILL,debuggee_pid, 0, 0);
}
...
JNIEXPORT void JNICALL Java_Platform_mdetach1
  (JNIEnv *env, jobject obj, jint debuggee_pid)
{
  int rc = ptrace(PTRACE_DETACH,debuggee_pid, 0, 0);
  if (rc==-1) {
  ...
  (JNIEnv *env, jobject obj, jint processID)
{
  ...
  rc = ptrace(PTRACE_ATTACH, processID, 0, 0, 0);
...
/**********************************************************
 * Return TRUE if the status word indicates a segmentation violation
 * Class:     Platform
 * Method:    isSegFault
 * Signature: (I)Z
 */
JNIEXPORT jboolean JNICALL Java_Platform_isSegFault
  (JNIEnv *env, jobject obj, jint status)
{
  int mysig = WSTOPSIG(status);
  if (mysig == SIGSEGV)
    return JNI_TRUE;
  else
    return JNI_FALSE;
  ...
  (JNIEnv *env, jobject obj, jint ReturnedValue)
{
  ...
  if (WIFSTOPPED(ReturnedValue)) {
    ...
    lookup_Linux_signal(buf, mysig);   /* fill the buffer with the signal description */
  } 
...
void lookup_Linux_signal(char *buf, int value)
{
  ...
  switch (value) {
    ...
    case SIGTRAP   : sprintf(buf,"Trace trap"); break;
    case SIGABRT   : sprintf(buf,"Abort"); break;
    case SIGBUS    : sprintf(buf,"BUS error"); break;
    case SIGFPE    : sprintf(buf,"Floating-point exception"); break;
    case SIGKILL   : sprintf(buf,"Kill, unblockable"); break;
    case SIGUSR1   : sprintf(buf,"User-defined signal 1"); break;
    case SIGSEGV   : sprintf(buf,"Segmentation violation"); break;
    ...
    case SIGTERM   : sprintf(buf,"Termination"); break;
    case SIGSTKFLT : sprintf(buf,"Stack fault"); break;
    case SIGCHLD   : sprintf(buf,"Child status has changed"); break;
    case SIGCONT   : sprintf(buf,"Continue"); break;
    case SIGSTOP   : sprintf(buf,"Stop, unblockable"); break;
    case SIGTSTP   : sprintf(buf,"Keyboard stop"); break;
    case SIGTTIN   : sprintf(buf,"Background read from tty"); break;
    case SIGTTOU   : sprintf(buf,"Background write to tty"); break;
    case SIGURG    : sprintf(buf,"Urgent condition on socket"); break;
    case SIGXCPU   : sprintf(buf,"CPU limit exceeded"); break;
    case SIGXFSZ   : sprintf(buf,"File size limit exceeded"); break;
    case SIGVTALRM : sprintf(buf,"Virtual alarm clock"); break;
    case SIGPROF   : sprintf(buf,"Profiling alarm clock"); break;
    case SIGWINCH  : sprintf(buf,"Window size change"); break;
    case SIGIO     : sprintf(buf,"I/O now possible"); break;
    case SIGPWR    : sprintf(buf,"Power failure restart"); break;
...
void print_wait_status(int ReturnedValue)
{
  ...
  if (WIFSTOPPED(ReturnedValue)) {
    ...
    if (mysig == SIGTRAP) {    /* check further if it's the debugger's own breakpoint */
      /* Ip = Java_Platform_readreg1(env, obj, debuggee_pid, jvmEIP); 
	 if (Ip != bp.next_addr && Ip !=bp.btarg_I) 
	 psignal(mysig,"Debuggee stopped by signal"); */
    } else {
      psignal(mysig,"Debuggee signal");
    }
  } else if (WIFEXITED(ReturnedValue))
