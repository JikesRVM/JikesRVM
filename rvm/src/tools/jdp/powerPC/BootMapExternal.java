...
class BootMapExternal extends BootMap {
  ...
  public int scanPrologSize(int startAddress) {
    int length = owner.mem.read(startAddress + VM_ObjectModel.getArrayLengthOffset() );
    for (int i = 0; i<length-1; i++) {
      int instr = owner.mem.read(startAddress + i*4);
      if (instr == PROLOGMARKER)
	return (i+1)*4;
    }
    return 0;
  }
  ...
  private int scanCompiledMethodIDTable(int address, int fieldOffset, boolean checkAddressRange) {
    ...
    // System.out.println("scanCompiledMethodIDTable: " + numMethods + " entries ...");
