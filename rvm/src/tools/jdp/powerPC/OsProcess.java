...
abstract class OsProcess implements jdpConstants, VM_BaselineConstants {
   ...
   public void pwait(boolean allThread) throws OsProcessException
  {
    ...
    // isIgnoredTrap() and isIgnoredOtherBreakpointTrap() will not reset the status
    // so the trap will be delivered to the debuggee
    while (isIgnoredTrap(status) || (skipBP=isLibraryLoadedTrap(status)) ||
	   isIgnoredOtherBreakpointTrap(status)) {
      // System.out.println("wait status = " + Integer.toHexString(status));
      if (skipBP)
  ...
  public String listAllThreads(boolean byClassName) {
	...
	result += "  -- -----------  -----------------\n";
      ...
      int activeThread = reg.registerToTPIndex(reg.hardwareTP());    // get index of active thread
      activeThread = getVMThreadForIndex(activeThread);         // convert index to thread pointer
  ...
  /**
   * List the threads current loaded and running in the AIX system threads
   * @return  
   * @exception
   * @see VM_Thread, VM_Scheduler
   */
