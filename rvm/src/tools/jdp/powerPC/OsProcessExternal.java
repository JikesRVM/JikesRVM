...
VM_ThinLockConstants {
   ...
   * @see
   */
  private void init(int fp, int sp, int tp, int jtoc, String[] gprName, String[] fprName, int tpshift) {
    Platform.setRegisterMapping(fp, sp, tp, jtoc, gprName, fprName, tpshift);
  }
  ...
  public boolean isIgnoredOtherBreakpointTrap(int status) {
	...
	int instr = mem.read(addr);
	if (instr==jdpConstants.BKPT_INSTRUCTION) {
	  // System.out.println("breakpoint trap status = " + Integer.toHexString(status) + 
	  //		     ", instr = " + Integer.toHexString(instr));
	  return false;        //  expected breakpoint trap, don't ignore
	} else {
	  return true;
	}
      } else {
    ...
    return false;  // relevant trap, don't ignore
  }
