...
class Emitters {
  ...
  public static void emitStuff(Class vmClass) {
    ...
    emitBootRecordDeclarations(vmClass);
    pln("#endif /* NEED_BOOT_RECORD_DECLARATIONS */");
    pln();

    pln("#ifdef NEED_BOOT_RECORD_INITIALIZATION");
    emitBootRecordInitialization(vmClass);
    pln("#endif /* NEED_BOOT_RECORD_INITIALIZATION */");
    pln();

    pln("#ifdef NEED_GNU_CLASSPATH_VERSION");
    // XXX Add reflection here to get it from the AlternateRealityClassLoader
    // XXX We now have a third case to take care of:
    ...
    // So, we get the version of the classpath library from gnu.classpath.configuration
    ...
    emitVirtualMachineDeclarations(vmClass, bootImageAddress);
    pln("#endif /* NEED_VIRTUAL_MACHINE_DECLARATIONS */");
    pln();

    pln("#ifdef NEED_ASSEMBLER_DECLARATIONS");
    emitAssemblerDeclarations(vmClass);
  ...
  static void emitBootRecordDeclarations () {
    ...
    try {
      bootRecord = VM_TypeReference.findOrCreate(VM_BootstrapClassLoader.getAlternateRealityClassLoader(), classDescriptor).resolve().asClass();
