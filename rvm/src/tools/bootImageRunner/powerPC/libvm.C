...
static const int addr_nibbles = (sizeof (void *) ) * 2;	// how many chars to
							// print out an
							// address in
							// hexadecimal form?
#ifdef PTRS_WITHOUT_PUNCTUATION
// Format for printing a Jikes RVM pointer.
#ifdef RVM_FOR_64_ADDR
#  define FMTrvmPTR "%08" PRIxPTR
#elif defined RVM_FOR_32_ADDR
...
#else  // ! PTRS_WITHOUT_PUNCTUATION

#ifdef RVM_FOR_32_ADDR
#  define FMTrvmPTR "%04" PRIxPTR "," "%04" PRIxPTR 
#define rvmPTR_ARG(p) ((uintptr_t) (p) >> 16) & (uintptr_t) 0xFFFF, ((uintptr_t) (p) & (uintptr_t) 0xFFFF) 
#elif defined RVM_FOR_64_ADDR
#  define FMTrvmPTR "%04" PRIxPTR ",%04" PRIxPTR ",%04" PRIxPTR ",%04" PRIxPTR
#define rvmPTR_ARG(p) (((uintptr_t) (p) >> 48) & (uintptr_t) 0xFFFF), (((uintptr_t) (p) >> 32) & (uintptr_t) 0xFFFF), (((uintptr_t) (p) >> 16) & (uintptr_t) 0xFFFF), (((uintptr_t) (p) & (uintptr_t) 0xFFFF)) 
#else
#  error "RVM_FOR_64_ADDR or RVM_FOR_32_ADDR must be defined"
#endif // RVM_FOR_XX_ADDR

#endif // PTRS_WITHOUT_PUNCTUATION

// We could also fix this by putting in an argument.  This, in turn, would
// let us deal with grouping the digits nicely.
...
cSignalHandler(int signum, int UNUSED zero, sigcontext *context) 
{
...
#endif
    // UNUSED: // unsigned jtoc =  save->gpr[VM_Constants_JTOC_POINTER];
   ...
   if (signum == SIGQUIT) { /* asynchronous signal used to awaken internal
       ...
       // interrupted C-library code, so we use 
       // boot image jtoc address (== VmToc) instead. 
       unsigned *flag = (unsigned *)((char *)VmToc + DebugRequestedOffset);
       if (*flag) {
	   fprintf(SysTraceFile, 
		   "%s: debug request already in progress, please wait\n", Me);
       } else {
	   fprintf(SysTraceFile, 
		   "%s: debug requested, waiting for a thread switch\n", Me);
	   *flag = 1;
       }
       return;
   }
...
createJVM(int vmInSeparateThread) 
{
   ...
   if (lib_verbose) {
       ...
       fprintf(SysTraceFile, "   initialHeapSize:      0x%08" PRIx32 "\n",   bootRecord.initialHeapSize);
       fprintf(SysTraceFile, "   maximumHeapSize:      0x%08" PRIx32 "\n",   bootRecord.maximumHeapSize);
...
#if (_AIX43 || defined RVM_FOR_LINUX)
	   sched_yield();
#else
	   pthread_yield();
#endif    
