...
/**
 * VCG implements the set of routines to output a graph
 * in VCG format.  The graph should implement the VCGGraph interface,
 * and the graph nodes and edges should implement VCGNode and
 * VCGEdge interfaces respectively.
 *
 * @see VCGGraph
 * @see VCGNode
 * @see VCGEdge
...
public final class VCG implements VCGConstants {
  /**
   * Returns a VCG representation for a given graph.
   * @param g the graph in question.
   */
  public static String toVCG(VCGGraph g) {
    VCGGraph.GraphDesc gd = g.getVCGDescriptor();
    ...
    Hashtable<VisNode, String> nodeNames = new Hashtable<VisNode, String>();
    int nodenum = 0;

    for (Enumeration<VCGNode> nodes = g.nodes(); nodes.hasMoreElements();) {
      VCGNode node = nodes.nextElement();
      VCGNode.NodeDesc nd = node.getVCGDescriptor();
      ...
      for (Enumeration<VisEdge> edges = node.edges(); edges.hasMoreElements();) {
        VCGEdge edge = (VCGEdge) edges.nextElement();
        VCGEdge.EdgeDesc ed = edge.getVCGDescriptor();
        String eName = edge.backEdge() ? "backedge" : "edge";
        res.append(indent(eName + ": {", 1));

        VisNode fromNode = edge.sourceNode();
        ...
        VisNode toNode = edge.targetNode();
   ...
   * @param g the graph in question.
   */
  public static void printVCG(String filename, VCGGraph g) {
   ...
   */
  public static void printVCG(String filename, VCGGraph g, boolean append) {
