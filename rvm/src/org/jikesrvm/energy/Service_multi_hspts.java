...
public class Service implements ProfilingTypes, ScalerOptions {
	...
	public static long start_ts = System.currentTimeMillis();
		...
		//public static void startFreqOptimization(int freq, int mid) {
...
//
			...
			if (mlen == 1 && thread.dvfsSliceExpired > RVMThread.FREQ || 
				...
				if (thread.skippedInvDvfs == 0) {
					// Store multiple hot method by method ID
//					String gov = Scaler.getGovernor();
//					thread.prevGov.put(mid, gov);

					thread.prevGov = Scaler.getGovernor();

					 VM.sysWriteln("startFreqOptimization: sample is taken. the current governor before setting is: " + thread.prevGov);
					// Store multiple hot method by method ID
//					if (gov.equalsIgnoreCase("userspace")) {
//						thread.prevFreq.put(mid, Scaler.checkCpuFrequency());
//					}
					if (thread.prevGov.equalsIgnoreCase("userspace")) {
						thread.prevFreq = Scaler.checkCpuFrequency();
					}
		 ...
		 */
	//	public static void resetGovAndFreq(RVMThread thread, int mid) {
		public static void resetGovAndFreq(RVMThread thread) {
		//	String targetGov = thread.prevGov.get(mid);
			String targetGov = thread.prevGov;

			VM.sysWriteln("endFreqOptimization: sample is taken. the previous governor before setting is: " + thread.prevGov);
			...
			if (targetGov != "") {
				...
				if (targetGov.equalsIgnoreCase("userspace")) {
					//Scaler.scale(thread.prevFreq.get(mid));
		 ...
		 */
		@Entrypoint
		//public static void endFreqOptimization(int mid) {
