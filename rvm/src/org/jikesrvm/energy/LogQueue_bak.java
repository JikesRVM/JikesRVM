...
public class LogQueue implements ProfilingTypes {
	...
	public static void initQueue(int socketNum) {
		if (logQueue == null) {
			logQueue = new LinkedList[LOG_SIZE];
		}
	}
	...
	public static void addLogQueue(int threadId, int methodId, double[] profileAttrs, long time, double hotMethodStartTime) {
		...
		if (logQueue[threadId] == null) {
			logQueue[threadId] = new LinkedList<LogEntry>();
		}
		logQueue[threadId].offer(entry);
	...
	public static void dumpLogQueue(String[] clsNameList, String[] methodNameList) {
		int count = 0;
		int i = 0;
		for (LinkedList<LogEntry> linkedList : logQueue) {
...
//			i++;
			...
			VM.sysWriteln("Size is here : " + linkedList.size());

			for (LogEntry entry : linkedList) {
