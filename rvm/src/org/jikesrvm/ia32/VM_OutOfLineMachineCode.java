...
public abstract class VM_OutOfLineMachineCode implements VM_BaselineConstants {
  ...
  private static ArchitectureSpecific.VM_CodeArray generateReflectiveMethodInvokerInstructions() {
    ...
    */
    ...
    if (SSE2_FULL) {
      ...
      asm.emitMOV_Reg_RegDisp(T1, T0, VM_ObjectModel.getArrayLengthOffset());    // T1 <- FPRs.length()
      ...
      fr_next = asm.forwardJcc(VM_Assembler.NE);
      asm.emitCVTSD2SS_Reg_Reg(XMM0, XMM0);
      fr_next.resolve(asm);

      asm.emitSUB_Reg_Imm(T1, 1);                         // length == 0 ?
      VM_ForwardReference fpr_r2 = asm.forwardJcc(VM_Assembler.EQ);
      ...
      asm.emitSUB_Reg_Imm(T1, 1);                         // length == 0 ?
      VM_ForwardReference fpr_r3 = asm.forwardJcc(VM_Assembler.EQ);
      ...
      asm.emitSUB_Reg_Imm(T1, 1);                         // length == 0 ?
      VM_ForwardReference fpr_r4 = asm.forwardJcc(VM_Assembler.EQ);
  ...
  private static ArchitectureSpecific.VM_CodeArray generateSaveThreadStateInstructions() {
    ...
    for (int i = 0; i < NUM_NONVOLATILE_GPRS; i++) {
      asm.emitMOV_RegDisp_Reg(S0,
                              Offset.fromIntZeroExtend(NONVOLATILE_GPRS[i] << LG_WORDSIZE),
  ...
  private static ArchitectureSpecific.VM_CodeArray generateThreadSwitchInstructions() {
    ...
    for (int i = 0; i < NUM_NONVOLATILE_GPRS; i++) {
      asm.emitMOV_RegDisp_Reg(S0,
                              Offset.fromIntZeroExtend(NONVOLATILE_GPRS[i] << LG_WORDSIZE),
                              NONVOLATILE_GPRS[i]); // T0.contextRegisters.gprs[i] := i'th register
    }

    // (2) Set currentThread.beingDispatched to false
    asm.emitMOV_RegDisp_Imm_Byte(T0,
                                 VM_Entrypoints.beingDispatchedField.getOffset(),
    ...
    for (int i = 0; i < NUM_NONVOLATILE_GPRS; i++) {
      asm.emitMOV_Reg_RegDisp(NONVOLATILE_GPRS[i],
                              S0,
                              Offset.fromIntZeroExtend(NONVOLATILE_GPRS[i] <<
                                                       LG_WORDSIZE)); // i'th register := restoreRegs.gprs[i]
    }
