...
final class JikesRVMSocketImpl extends SocketImpl implements VM_SizeConstants{
     ...
     */
    protected synchronized void create(boolean streaming)
	...
	VM_BootRecord bootRecord = VM_BootRecord.the_boot_record;
	if (VM_SysCall.call2(bootRecord.sysNetSocketShutdownIP, native_fd, CLOSE_INPUT) != 0)
	...
	if (native_fd == -1) throw new IOException("socket already closed");
	VM_BootRecord bootRecord = VM_BootRecord.the_boot_record;
	if (VM_SysCall.call2(bootRecord.sysNetSocketShutdownIP, native_fd, CLOSE_OUTPUT) != 0)
	    throw new IOException("could not close input side of socket");
    }
     ...
     * @exception	IOException	if an error occurs while connecting
     */
    private void connectInternal(InetAddress remoteAddr, int remotePort, int timeout)
	throws IOException 
    {
	VM_BootRecord bootRecord = VM_BootRecord.the_boot_record;
	int rc = -1;
