...
public class VM_DynamicLibrary
{
  ...
  // Taken:    library name
  // Returned: library system handler (-1: not found or couldn't be created)
  //
  public VM_DynamicLibrary(String libraryName)
  {
    ...
    // make sure we have enough stack to load the library.  This operation has been
    ...
    libHandler = VM.sysCall1(bootRecord.sysDlopenIP, VM_Magic.objectAsAddress(asciiName));
    // UNPIN(asciiName);
  ...
  // Taken:    symbol name
  // Returned: symbol system handler (actually an address to an AixLinkage triplet)
    ...
    getSymbol(String symbolName)
  {
    ...
    // PIN(asciiName);
    VM_BootRecord bootRecord = VM_BootRecord.the_boot_record;
    int address = VM.sysCall2(bootRecord.sysDlsymIP, libHandler, VM_Magic.objectAsAddress(asciiName));
    // UNPIN(asciiName);
