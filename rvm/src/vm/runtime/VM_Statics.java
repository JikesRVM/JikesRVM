...
public class VM_Statics implements VM_Constants
   ...
   // Taken:       literal value (as bits)
   // Returned:    slot number that was allocated
   // Side effect: literal value is stored into jtoc
   //
   static int
   findOrCreateIntLiteral(int literal)
      {
   ...
   //
   static int
   findOrCreateFloatLiteral(int literal)
      {
   ...
   // Taken:       literal value (as bits)
   // Returned:    slot number of first of two slots that were allocated
   // Side effect: literal value is stored into jtoc
   //
   static int
   findOrCreateLongLiteral(long literal)
      {
   ...
   //
   static int
   findOrCreateDoubleLiteral(long literal)
      {
   ...
   //
   static int
   findOrCreateStringLiteral(VM_Atom literal)
   ...
   // Taken:    description of a static field or method (see "kinds", above)
   // Returned: slot number that was allocated (two slots are allocated for longs and doubles)
   //
   static int
   allocateSlot(byte description)
      {
      int slot = nextSlot;
   ...
   // Taken:    slot number obtained from allocateSlot()
   // Returned: true --> slot contains a reference
   //
   static boolean
   ...
   // Taken:    slot number obtained from allocateSlot()
   // Returned: description of slot contents (see "kinds", above)
   //
   static byte
   ...
   //
   static String 
   getSlotDescriptionAsString(int slot)
      {
      String kind = null;
   ...
   //
   public static long
   getSlotContentsAsLong(int slot)
      {	
	//-#if RVM_FOR_IA32
   ...
   getSlotContentsAsObject(int slot)
      {
   ...
   //
   public static void
   setSlotContents(int slot, int value)
      {
      slots[slot] = value;
   ...
   //
   public static void
   setSlotContents(int slot, long value)
      {
	//-#if RVM_FOR_IA32
   ...
   //
   static void
   setSlotContents(int slot, Object object)
      {
   ...
   private static int  slots[];         // static data values (pointed to by jtoc register)
   private static byte descriptions[];  // corresponding descriptions (see "kinds", above)
   private static int  nextSlot;        // next available slot number
   private static final int INITIAL_SLOTS = 32768; // initial size of slots[] and descriptions[]
   ...
   init()
      ...
      // In particular, this ensures that identity of string literals is preserved
      // across constant pools (ie. so that "abc" == "abc" always yields "true").
      //
