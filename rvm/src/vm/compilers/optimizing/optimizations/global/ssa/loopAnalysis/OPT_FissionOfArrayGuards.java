 ...
 *
 * 4) Create a clone of the original loop that uses the first of the
 * newly created registers instead of the original registers.
 *
 * 5) Create a second clone, this time with the result of the
 ...
 * @author Ian Rogers
 */
public final class OPT_FissionOfArrayGuards extends OPT_CompilerPhase implements OPT_Operators, OPT_Constants{
  ...
  /**
	...
	* @return "Loop Fission of Array Guards"
	*/
  ...
  /**
	* Optimized loop phi node operand number
	*/
  private final static int OPTIMIZED_LOOP_OPERAND = 0;
  /**
	* Unoptimized loop phi node operand number
	*/
  ...
  private boolean findLoopToOptimise(OPT_AnnotatedLSTNode loop) {
	 ...
	 if (checksToEliminate.size() == 0) {
		...
		// 4) Create a clone of the original loop that uses the first of
		...
		OPT_BasicBlock firstBranchBlock = loop.header.createSubBlock(SYNTH_ARRAYGUARD_FISSION_BCI, ir);
  ...
  private void generatePhiNodes(OPT_AnnotatedLSTNode loop, ArrayList registers, ArrayList types, ArrayList phiInstructions, Map subOptimalRegMap, Map optimalRegMap) {
	 ...
	 for (int i=0; i < registers.size(); i++) {
		...
		phi.setBytecodeIndex(SYNTH_ARRAYGUARD_FISSION_BCI);
  ...
  private Map createCloneLoop(OPT_AnnotatedLSTNode loop, Map regMap, Map regToBlockMap) {
	 ...
	 OPT_BasicBlock new_pred = loop.header.createSubBlock(SYNTH_ARRAYGUARD_FISSION_BCI, ir);
  ...
  private boolean createBranchBlocks(OPT_AnnotatedLSTNode loop, OPT_BasicBlock block, ArrayList checksToEliminate,
	 ...
	 if(loop.isMonotonic()) {
		...
		if (loop.terminalIteratorValue.isIntConstant()) {
		  ...
		  tempInstr.setBytecodeIndex(SYNTH_ARRAYGUARD_FISSION_BCI);
	 ...
	 // 3) Finish up with goto and generate true guard value
	 {
		...
		branch.setBytecodeIndex(SYNTH_ARRAYGUARD_FISSION_BCI);
		...
		if(NullCheck.conforms(instr)) {
		  ...
		  if (ref.isRegister() && refToGuardMap.containsKey(ref.asRegister().register)) {
			 ...
			 if (guard != null) {
				...
				OPT_BasicBlock new_block = block.createSubBlock(SYNTH_ARRAYGUARD_FISSION_BCI, ir);
  ...
  private OPT_BasicBlock generateExplicitBoundCheck(OPT_Instruction boundCheckInstr,
	 ...
	 if (lowerBoundTestRedundant && upperBoundTestRedundant) {
		...
		move.setBytecodeIndex(SYNTH_ARRAYGUARD_FISSION_BCI);
		...
		array_length_instr.setBytecodeIndex(SYNTH_ARRAYGUARD_FISSION_BCI);
			 ...
			 for (int index=0; index<Phi.getNumberOfPreds(origInstr); index++) {
				...
				if (predOp.block == loop.exit) {
				  if (fixed) { // We've tried to do 2 replaces => something wrong
					 OPT_SSA.printInstructions(ir);
					 OPT_OptimizingCompilerException.UNREACHABLE("OPT_FissionOfArrayGuards",
				  ...
				  if (rval.isRegister()) {
					 ...
					 OPT_OptimizingCompilerException.UNREACHABLE("OPT_FissionOfArrayGuard", "Unknown operand type", rval.toString());
				  }
				  fixed = true;
				}
			 }
