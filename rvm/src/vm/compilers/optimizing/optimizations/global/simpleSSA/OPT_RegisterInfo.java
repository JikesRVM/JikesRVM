...
/**
 * This class computes register lists and associated information.
 * A register list holds the defs and uses (operands) for each register.
 *
 * @author Vivek Sarkar
 * @author Stephen Fink
 * @author Dave Grove
 ...
 */
final class OPT_RegisterInfo implements OPT_Operators {
  final static boolean DEBUG = false;
  final static boolean TRACE_RL_ACTIONS = false;
   ...
   */
  static void clearRegisterList(OPT_IR ir) {
    ...
    if (TRACE_RL_ACTIONS || DEBUG)
      VM.sysWrite("Cleared RegisterList\n");
  }
   ...
   * @param ir the IR in question
   */
  static void computeRegisterList(OPT_IR ir) {
    // Clear old register list (if any)
    clearRegisterList(ir);
    if (TRACE_RL_ACTIONS || DEBUG)
   ...
   */
  static void removeInstructionAndUpdateRegisterLists(OPT_Instruction s) {
   ...
   * instruction s
   */
  static void updateRegisterListsForNewInstruction(OPT_Instruction s) {
    for (OPT_OperandEnumeration e = s.getDefs(); e.hasMoreElements();) {
      OPT_Operand op = e.next();
      if (op != null && op.isRegister() && 
          !op.asRegister().register.isPhysical()) {
   ...
   * @param ir the IR in question
   */
  static void printRegisterList(OPT_IR ir) {
    OPT_Compiler.header("RegisterList", ir.method);
    for (OPT_Register reg = ir.regpool.getFirstRegister(); 
        reg != null; reg = reg.getNext()) {
      printDefs(reg);
      printUses(reg);
   ...
   * list.
   * NOTE: the RegisterList MUST be computed BEFORE calling this function
   *
   * @param ir the IR in question
   */
   ...
   * Remove reg2 from the register lists.
   */
