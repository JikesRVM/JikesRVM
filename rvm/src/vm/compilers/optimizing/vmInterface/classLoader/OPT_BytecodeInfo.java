...
class OPT_BytecodeInfo {
  ...
  private byte[] bcodes;
  /**
   * A cursor into the constant pool
   */
   ...
   */
  final OPT_Operand getConstantOperand (int opcode) {
    int JBC_ldc = 0x12;
    int i = (opcode == JBC_ldc) ? fetch1ByteUnsigned() : fetch2BytesUnsigned();
    byte desc = declaringClass.getLiteralDescription(i);
    switch (desc) {
      case VM_Statics.INT_LITERAL:
        return  OPT_ClassLoaderProxy.proxy.getIntFromConstantPool(
                                        declaringClass, i);
      case VM_Statics.FLOAT_LITERAL:
        return  OPT_ClassLoaderProxy.proxy.getFloatFromConstantPool(
                                        declaringClass, i);
      case VM_Statics.STRING_LITERAL:
        return  OPT_ClassLoaderProxy.proxy.getStringFromConstantPool(
                                        declaringClass, i);
      case VM_Statics.LONG_LITERAL:
        return  OPT_ClassLoaderProxy.proxy.getLongFromConstantPool(
                                        declaringClass, i);
      case VM_Statics.DOUBLE_LITERAL:
        return  OPT_ClassLoaderProxy.proxy.getDoubleFromConstantPool(
                                        declaringClass, i);
      default:
        {
  ...
  final VM_Field getFieldReference () {
    constantPoolIndex = fetch2BytesUnsigned();
  ...
  final VM_Method getMethodReference () {
    constantPoolIndex = fetch2BytesUnsigned();
    return  declaringClass.getMethodRef(constantPoolIndex);
  }
