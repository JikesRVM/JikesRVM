 ...
 */
/**
 * An implementation of {@link OPT_ClassLoaderProxy} for Jalapeno.
 ...
 * @author Dave Grove
 **/
public final class OPT_JalapenoClassLoaderProxy extends OPT_ClassLoaderProxy {

  /**
   * Default constructor.
   */
  public OPT_JalapenoClassLoaderProxy () {
  ...
  public VM_Array createArray (VM_Atom des) {
    throw  new OPT_OptimizingCompilerException("must not be called in Jalapeno -- all VM_Array instances are created by VM_ClassLoader");
  }
  ...
  public VM_Class createClass (VM_Atom des) {
    throw  new OPT_OptimizingCompilerException("must not be called in Jalapeno -- all VM_Class instances are created by VM_ClassLoader");
  }
  ...
  public VM_Primitive createPrimitive (VM_Atom des) {
    throw  new OPT_OptimizingCompilerException("must not be called in Jalapeno -- all VM_Primitive instances are created by VM_ClassLoader");
  }
  ...
  public OPT_StringConstantOperand getStringFromConstantPool (VM_Class klass, 
							      int index) {
    int offset = klass.getLiteralOffset(index) >> 2;
    StringWrapper str = new JalapenoStringWrapper(offset);
    return  new OPT_StringConstantOperand(str);
  }

  /**
   * A wrapper around String constants in the Jalapeno VM
   */
  public static final class JalapenoStringWrapper extends StringWrapper {
    private int offset;         // offset in JTOC

    /**
     * @param     int off JTOC offset of the STring constant
     */
    JalapenoStringWrapper (int off) {
      offset = off;
    }

    public boolean equals (Object str) {
      if (!(str instanceof JalapenoStringWrapper))
        return  false;
      return  offset == ((JalapenoStringWrapper)str).offset;
    }
