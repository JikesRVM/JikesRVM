...
public class WorkQueue implements VM_Uninterruptible {
  ...
  public synchronized void initialSetup (int n) throws VM_PragmaLogicallyUninterruptible {
    ...
    if ( ! VM.BuildForSingleVirtualProcessor) {
      ...
      if(trace)
	VM_Scheduler.trace("WorkQueue.initialSetup:","numRealProcessors =",numRealProcessors);
    }
  }
  ...
  void reset () throws VM_PragmaUninterruptible {
    ...
    if (trace) VM_Scheduler.trace("WorkQueue.reset:","numThreadsWaiting =",
				  numThreadsWaiting);
	...
	if ( (++debug_counter % 100000) == 0) {
	  VM_Scheduler.trace("WorkQueue","waiting - numThreadsWaiting =",
			     numThreadsWaiting);
  ...
  static void
    ...
    if (myThread.putBufferStart.isZero()) 
      WorkQueue.allocatePutBuffer(myThread);
    else {
    ...
    if (!myThread.getBufferStart.isZero()) {
      WorkQueue.freeGetBuffer(myThread);  // release old work q buffer
  ...
  VM_Address getBufferAndWait () throws VM_PragmaLogicallyUninterruptible, VM_PragmaUninterruptible {
    ...
    while (true) {
      ...
      if (debug) {
	if ( (++debug_counter % 100000) == 0) {
	  VM_Scheduler.trace("WorkQueue","waiting - numThreadsWaiting =",numThreadsWaiting);
	  if ( ++debug_counter_counter > 10)
	    VM.shutdown(-2);
	}
      }
  ...
  static void putToWorkBuffer (VM_Address ref ) throws VM_PragmaUninterruptible {
    ...
    if (myThread.putBufferTop.EQ(myThread.putBufferStart)) {
      ...
      WorkQueue.workQueue.addBuffer(myThread.putBufferStart);

      WorkQueue.allocatePutBuffer(myThread);    // allocate new Put Buffer
    }
  }
  ...
  public static VM_Address getFromWorkBuffer () throws VM_PragmaUninterruptible {
	...
	WorkQueue.allocatePutBuffer(myThread);  //get a new Put Buffer
      }
	...
	myThread.bufferWaitTime += (VM_Time.now() - temptime); 
      ...
      WorkQueue.freeGetBuffer(myThread);  // release the old work q buffer
      myThread.getBufferStart = newbufaddress;
  ...
  private static void
    ...
    if (!myThread.extraBuffer.isZero()) {
      ...
      bufferAddress = VM_Interface.malloc(WORK_BUFFER_SIZE);
      if (bufferAddress.isZero()) {
	VM.sysWrite(" In WorkQueue: call to sysMalloc for work buffer returned 0\n");
	VM.shutdown(1901);
      }
    }
  ...
  private static void freeGetBuffer (VM_CollectorThread myThread) throws VM_PragmaUninterruptible { 
    ...
    if (myThread.extraBuffer.isZero()) 
      ...
      VM_Interface.free(myThread.getBufferStart);
  ...
  static void emptyWorkQueue() throws VM_PragmaUninterruptible {
      ...
      VM_Address ref = WorkQueue.getFromWorkBuffer();
  ...
  public static void resetCounters ( VM_CollectorThread ct ) throws VM_PragmaUninterruptible {
    ct.copyCount = 0;
    ct.rootWorkCount = 0;
    ct.putWorkCount = 0;
    ct.getWorkCount = 0;
  ...
  public static void saveAllCounters() throws VM_PragmaUninterruptible {
    int i;
