 ...
 */
// Multiplex execution of large number of VM_Threads on small number of o/s kernel threads.
// 31 August 1998 Bowen Alpern & Derek Lieber
//
final class VM_Processor implements VM_Uninterruptible,  VM_Constants, VM_BaselineConstants, VM_GCConstants {
  ...
  // Create data object to be associated with an o/s kernel thread (aka "virtual cpu" or "pthread").
  // Taken: id that will be returned by getCurrentProcessorId() for this processor.
  //
  VM_Processor (int id) {

    //-#if RVM_FOR_IA32
    // presave JTOC register contents (so lintel compiler can us JTOC for scratch)
    ...
    // allocate & initialize collection (write buffers..) and allocation data structures
  ...
  VM_Processor (int id,  boolean nativeProcessor ) {
    ...
    //-#if RVM_FOR_IA32 // presave JTOC register contents (so lintel compiler can us JTOC for scratch)
    if (VM.runningVM) jtoc = VM_Magic.getJTOC();
    //-#endif
  ...
  void enableThreadSwitching () {
    ...
    if (VM.VerifyAssertions) VM_Scheduler.assert(threadSwitchingEnabledCount <= 1);
    if (VM.VerifyAssertions && VM_Collector.gcInProgress() && !VM.BuildForConcurrentGC)
      VM.assert(threadSwitchingEnabledCount <1 || getCurrentProcessorId()==0);
    if (threadSwitchingEnabled() && threadSwitchPending) { // re-enable a deferred thread switch
  ...
  //       2. The number words of arguments to this method must match the number of words of
  //          arguments to VM_Magic.saveThreadState(). Furthermore, a call to 
  //          VM_Magic.saveThreadState() must immediately preceed the call to this method.
  //
  void dispatch () {
    ...
    // if we've got too much work, transfer some of it to another processor that has nothing to do
    //
    if (VM_Processor.idleProcessor != null && !readyQueue.isEmpty()
		&& VM_Processor.getCurrentProcessor().processorMode != NATIVEDAEMON ) {
...
//-#endif
    ...
    if (VM.BuildForCpuMonitoring) {
      ...
      if (previousThread.cpuStartTime == -1) ; // primordial thread: ignore first time slice
...
//-#endif
  ...
  static VM_Processor idleProcessor;      // non-null --> a processor that has no work to do
  int                 nextTransferId = 1; // id of next processor to receive load balancing
  ...
  void scheduleThread (VM_Thread t) {
    ...
    if (VM.BuildForConcurrentGC) { // currently don't move threads around - so keep on same processor
  ...
  // Find a thread that can be run by this processor and remove it from its queue.
  //
  private VM_Thread getRunnableThread() {
	...
	///	transferQueue.enqueue(t);
	transferMutex.unlock();
	// for default jni implementation - if running is stack of a native idle thread, do not spinwait here,
	// instead proceed on to other threads, the idle thread if necessary, to free
	// up the native idle thread so it can be dispatched on its native processor
    ...
    if (!readyQueue.isEmpty()) {
      ...
      if (VM.VerifyAssertions) VM_Scheduler.assert(t.beingDispatched == false || t == VM_Thread.getCurrentThread()); // local queue: no other dispatcher should be running on thread's stack
      return t;
    }
    ...
    // if the idle thread isn't on the idle queue, it must be on the transfer queue
    if (!transferQueue.isEmpty()) {
      transferMutex.lock();
      VM_Thread t = transferQueue.dequeue();
      while (t.beingDispatched && t != VM_Thread.getCurrentThread()) { // thread's stack in use by some OTHER dispatcher
	if (trace) VM_Scheduler.trace("VM_Processor", "getRunnableThread: stack in use", t.getIndex());
	transferQueue.enqueue(t);
  ...
  void    dumpProcessorState() {
    ...
    VM_Scheduler.writeString(" mode: "); dumpMode(); VM_Scheduler.writeString("\n");
    //-#if RVM_WITH_DEDICATED_NATIVE_PROCESSORS // alternate implementation of jni
    //-#else                                    // default implementation of jni
    VM_Scheduler.writeString(" status: "); dumpStatus(); VM_Scheduler.writeString("\n");
    VM_Scheduler.writeString(" threadSwitchRequested: ");
    VM_Scheduler.writeDecimal(threadSwitchRequested); VM_Scheduler.writeString("\n");
    //-#endif
  }
  ...
  static VM_Processor createNativeProcessorForExistingOSThread(VM_Thread withThisThread) {
    ...
    // Because the start up thread will not be executing to initialize itself as in the
...
// alternate implementation of jni
  ...
  static VM_Processor createNativeProcessor () {
    ...
    //
    if (trace) 
      VM_Scheduler.trace("VM_Processor", "starting native virtual cpu pthread ");
	...
	if(VM_Magic.attempt(VM_BootRecord.the_boot_record, 
			    VM_Entrypoints.lockoutProcessorOffset,
			    VM_Magic.objectAsAddress(VM_Magic.getProcessorRegister()),
			    0, lockoutId))
	  break;
...
// alternate implementation of jni
  ...
  public int  setGCState () {
    ...
    }while (! (VM_Magic.attempt(this, VM_Entrypoints.vpStateOffset, oldState, newState)));
    return oldState;
  }
  ...
  public int  resetGCState () {
    ...
    do{
      oldState = VM_Magic.prepare(this, VM_Entrypoints.vpStateOffset);
      if (VM.VerifyAssertions) VM.assert((oldState & GC_IN_PROGRESS) ==  GC_IN_PROGRESS) ;
      newState = oldState & (~GC_IN_PROGRESS);
    }while (! (VM_Magic.attempt(this, VM_Entrypoints.vpStateOffset, oldState, newState)));
    return oldState;
  }
...
// default implementation of jni
  ...
  public boolean lockInCIfInC () {
    ...
    do {
      ...
      if (oldState != IN_NATIVE) {
        if (VM.VerifyAssertions) VM.assert((oldState==IN_JAVA)||(oldState==IN_SIGWAIT)) ;
        result = false;
        break;
      }
      newState = BLOCKED_IN_NATIVE;
    } while (!(VM_Magic.attempt(VM_Magic.addressAsObject(vpStatusAddress), 0, oldState, newState)));
    return result;
  }
  ...
  public boolean blockInWaitIfInWait () {
    ...
    } while (!(VM_Magic.attempt(VM_Magic.addressAsObject(vpStatusAddress), 0, oldState, BLOCKED_IN_SIGWAIT)));
    return result;
  }
...
//-#endif
  ...
  //       2. id must be non-zero because it is used in VM_ProcessorLock ownership tests
  //
  int id;
  ...
  // Is it time for this processor's currently running VM_Thread to call its "threadSwitch" method?
  // A value of: 
  //    -1 means yes
  //     0 means no
  ...
  // Should this processor dispatch a new VM_Thread when "threadSwitch" is called?
  // Also used to decide if it's safe to call yield() when contending for a lock.
  // A value of:
  ...
  // Was "threadSwitch" called while this processor had thread switching disabled?
  //
  boolean threadSwitchPending;

  VM_Thread        activeThread;    // thread currently running on this processor
  VM_Thread        previousThread;  // thread previously running on this processor

  VM_ThreadQueue   transferQueue; // threads to be added to ready queue
  VM_ProcessorLock transferMutex; // guard for above

  VM_ThreadQueue   readyQueue;    // threads waiting for a timeslice in which to run
  VM_ThreadIOQueue ioQueue;       // threads waiting for i/o
  VM_ThreadQueue   idleQueue;     // thread to run when nothing else to do

  // The following are conditionalized by "if (VM.VerifyAssertions)"
  //
  int              lockCount;     // number of processor locks currently held (for assertion checking)
  ...
  //
  int    localCurrentAddress;  // current position within current allocation buffer
  int    localEndAddress;      // end (1 byte beyond) of current allocation buffer

  // pointers for allocation from processor local "chunks" or "ToSpace" memory,
  // for copying live objects during GC.
  //
  int    localMatureCurrentAddress;   // current position within current "chunk"
  int    localMatureEndAddress;       // end (1 byte beyond) of current "chunk"
...
//-#if RVM_WITH_DEDICATED_NATIVE_PROCESSORS
// alternate implementation of jni
  int     vpState;                     // state field in each VM_Processor
//-#else
// default implementation of jni
  ...
  int   vpStatusIndex;            // index of this processors status word in vpStatus array
  int   vpStatusAddress;          // address of this processors status word in vpStatus array
//-#endif
  ...
  Object jtoc;
  int    threadId;
  int    framePointer;        // FP for current frame (opt compiler wants FP register) TODO warn GC about this guy
  int    hiddenSignatureId;   // "hidden parameter" for interface invocation thru the IMT
  int    arrayIndexTrapParam; // "hidden parameter" from ArrayIndexOutOfBounds trap to C trap handler
  //-#endif
