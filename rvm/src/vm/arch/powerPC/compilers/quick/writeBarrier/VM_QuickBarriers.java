...
class VM_QuickBarriers implements VM_BaselineConstants {
  ...
  static void compilePutstaticBarrier (VM_Assembler asm, int fieldOffset,
    ...
    asm.emitMTLR(T0);
    // Load the offset into T0 -- code based on asm.emitCALtoc
    if ((fieldOffset & 0x8000) != 0) {
      asm.emitCAU (T0, (fieldOffset>>16) + 1);
      asm.emitCAL (T0, fieldOffset|0xFFFF0000, T0);
    } else {
      asm.emitCAU (T0, fieldOffset>>16);
      asm.emitCAL (T0, fieldOffset&0xFFFF, T0);
    }
    if (rValue != T1)
