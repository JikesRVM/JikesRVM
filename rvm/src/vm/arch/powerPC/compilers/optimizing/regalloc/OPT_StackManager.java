...
public final class OPT_StackManager extends OPT_GenericStackManager
  ...
  final OPT_Instruction makeMoveInstruction(OPT_Register lhs, 
    ...
    if (rhs.isFloatingPoint() && lhs.isFloatingPoint()) {
      ...
      //-#endif
      //-#if RVM_FOR_64_ADDR
      return MIR_Move.create(PPC_MOVE, L(lhs), L(rhs));
      //-#endif
    } else
  ...
  final private void insertEpilogue(OPT_Instruction ret) {
    ...
    //-#endif
    //-#if RVM_FOR_64_ADDR
    ret.insertBack(MIR_Move.create(PPC_MTSPR, L(phys.getLR()),
                                   L(phys.getTemp())));
    //-#endif
  ...
  final void insertNormalPrologue() {
    ...
    //-#endif
    //-#if RVM_FOR_64_ADDR
    ptr.insertBefore(MIR_Move.create(PPC_MFSPR, L(R0),
                                     L(phys.getLR()))); // 1
    //-#endif   
  ...
  final void insertExceptionalPrologue () {
    ...
    if (stackOverflow) {
      ...
      //-#endif
      //-#if RVM_FOR_64_ADDR
      MIR_Trap.mutate(ptr, PPC64_TD, OPT_PowerPCTrapOperand.LESS(), L(FP), L(R0),
                      OPT_TrapCodeOperand.StackOverflow());
      //-#endif
