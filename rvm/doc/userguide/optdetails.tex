...
implementation details for the \jp\ optimizing compiler.
...
The \jp\ build process generates the {\tt OPT\_Options.java} 
...
For example, \jp\ supports a choice of three options for floating-point
...
So, by default, \jp\ uses the {\em strict} floating-point semantics.  To use
...
\index{LIR}
\index{MIR}
The fundamental unit for optimization in \jp\ is a single method. 
...
{\tt \$RVM\_ROOT/optimizingCompiler/IR/InstructionFormatList.dat}.
...
{\tt vm/arch/\{arch\}/optimizingCompiler/ConvertLIR2MIR/OPT\_BURS\_Helpers.java}.
For more information on the BURS algorithm see
{\tt vm/optimizingCompiler/ConvertLIR2MIR/OPT\_BURS.java}.
