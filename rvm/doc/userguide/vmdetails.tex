...
implementation details for the \JikesTM{} RVM runtime system.
...
operations while minimizing storage overhead. \JikesTM{} RVM's
object model 
is defined by \xlink{{\tt VM\_ObjectModel.java}}{\VMObjectModelURL}.

\paragraph{Overview}
Values in the \JavaTM{} programming language are either {\em
...
The \AIXTM\ operating system permits loads from low memory,
but accesses 
...
each of \JikesTM{} RVM's global data structures, as well as
...
%\htmlonly{\subsubsection{\AIXTM{} VM Conventions}} 
%\texonly{\subsubsection{\AIXTM{} VM Conventions}} 
% \subsubsection{\AIXTM\ VM Conventions} 
\subsubsection{\AIXTMheading{} VM Conventions}
% \subsubsection{AIX VM Conventions} 
...
\item Check to see if the \JavaTM{} thread must yield the {\tt VM\_Processor}
...
\subsubsection{LinuxRheading/IA32 VM Conventions} \label{lintel-conventions}
\index{stack conventions}
\index{register conventions}
\index{calling conventions}

This section describes register, stack, and calling conventions that
apply to Jikes RVM on \LinuxR{}/IA32.  
...
\JikesTM{} RVM implements the \JavaTM{} programming
