...
\JikesTM{} RVM can be used to profile an application and the VM
...
\PowerPCTM/\AIXTM\ version 5, but can be
...
are generated for each virtual processor, and each \JavaTM{} thread that executes.
For example, the aggregate HPM counter values for virtual processor 1
when Jikes RVM is run on the \PowerPCTM{} POWER4 architecture with group 23 
...
use the {\tt production} configuration.  See Section~\ref{section:installation}
\begin{verbatim}
