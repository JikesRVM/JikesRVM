...
hatch that \JikesTM{} RVM provides to implement
functionality that is not 
possible using the pure \JavaTM{} programming language.  For example, the Jikes RVM garbage
collectors and 
...
\JikesTM{} RVM implementor must be {\em extremely careful} when
...
Since magic is inexpressible in the Java programming language , it is unsurprising that the
