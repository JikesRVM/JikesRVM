...
\link{finally discuss further installation steps}{sub-sub-sec:InstallationSteps} for
...
(e.g., build on Linux/x86 for an AIX\TMweb/PowerPC\TMweb
...
``\texttt{\$RVM\_\-ROOT\-/\-rvm\-/\-con\-fig\-/\-build}''.  Follow the examples in this
...
To go into agonizing depth, except for the special configurations
(\texttt{development}, 
\texttt{prototype}, \texttt{production}, \texttt{prototype-opt}), and
for a couple of historical ones, standard Jikes RVM configuration
files follow the following naming scheme:
\begin{verse}
       \MZeroOrOne{\Mlitch{ExtremeAssertions}} \MZeroOrOne{\Mlitch{Fast} \MZeroOrOne{\Mlitch{Opt} \MZeroOrOne{\Mlitch{0}\Mor\Mlitch{1}}} \Mor{} \Mlitch{Full}} \Mmeta{boot image compiler} 
       \Mlbr{} \Mlitch{Base} \Mor{} \Mlitch{Adaptive} \Mrbr{}
...
\item[\IndexTexttt{RVM\_WITHOUT\_PREEMPTIVE\_THREAD\_SWITCHING}]
  Thread switch only at explicit yield and wait calls.

\item[\IndexTexttt{RVM\_WITH\_ALL\_CLASSES}]  Include all VM classes
  in bootimage, not just those needed for startup.  
  The \Mlitch{Fast} and \Mlitch{Full} set this flag.

\item[\IndexTexttt{RVM\_WITHOUT\_ASSERTIONS}]
 Exclude runtime assertion checks.  This is what the \Mlitch{Fast}
 configurations set.

\item[\IndexTexttt{RVM\_WITHOUT\_AOS\_LOG}] Turn off \link{AOS
  logging}[ (discussed in Section~\Ref, on page~\Pageref]{aos-logging}.
