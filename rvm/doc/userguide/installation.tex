...
You can use any of the pre-compiled binaries, or build it yourself
from the source. We recommend either version 1.13 or version 1.18.  
We have seen problems with version 1.14 through 1.17.  For releases of
Jikes RVM before version 2.2.0, you must use jikes 1.13. Note that if you
use version 1.18 you will need to give it the additional command line
argument {\tt -target 1.3} to ensure that Jikes RVM will be able to
...
\item a JDK.  We currently recommend the Blackdown VM, v1.3.1. You can
download this from 
\xlink{{\tt \BlackdownURL}} {\BlackdownURL}. 
We have seen problems using the IBM 1.3.1 DK; avoid it until these problems
can be resolved.
\item {\tt ksh}. This must be installed in {\tt /bin/ksh}. If you
don't already have a ksh, we recommend {\tt pdksh}.
\end{itemize}
...
Classpath license is available at
\xlink{{\tt \classpathURL}}{\classpathURL}.

The first and second items are available  from the Jikes RVM
\xlink{download}{\RVMDownloadURL} page. The third item is available at
\xlink{{\tt \classpathURL}}{\classpathURL}.
You do not always need to explicitly download the
...
\xlink{{\tt Jikes}}{ \jikesURL } fail to compile the
