This section gives instructions on how to install and run \JikesTM{} RVM.
...
\subsection{Installing on \RedHatTMheading{} \LinuxRheading{} 7.3, 8.x, and 9.x, or \SuSERheading{} Linux 8.x for \IntelRheading{} x86}

\RedHatTM{} \LinuxR{} for \IntelR{} IA32 (Intel x86) has emerged as the 
primary platform for Jikes RVM.  \SuSER{} Linux is also in daily use
...
\item Download and install a \JavaTM{} Virtual Machine if you have not
...
\xlink{{\tt \BlackdownURL}} {\BlackdownURL}. 
...
JVMs that are known to fail on Linux/IA32 include the \IBMR{} 1.3.1 and
...
\subsection{The Hard Way}
 ...
 For any platform that is not SuSE Linux 8.2 or newer, or Red Hat Linux 7.3 or newer for Intel
...
Jikes Compiler & 1.18+ or 1.13 & \xlink{\tt \jikesURLHyphenated}{\jikesURL}       \\
...
\hline \multicolumn{3}{|c|}{\em \AIXTM{}/\PowerPCTM{}}                     \\ \hline

\AIXTM{}            & 4.3+     & %   \center{---}                      
\\
JDK            & \IBMR{} DK 1.3.0 or 1.4.0 & \xlink{\tt \AIXJdkURL}{\AIXJdkURL} \\ 
...
GNU Classpath libaries from GNU Classpath's CVS.  If you instead download a
tagged pre-built GNU Classpath release (we recommend this), then you don't need
these.  Please see section~\ref{manual-classpath-root}.
...
To install and build \JikesTM{} RVM, two items are required
\begin{itemize}
...
First extract the \JikesTM{} RVM source distribution into a
directory such as  
{\tt \$HOME/rvmRoot}.
\begin{verbatim}
% cd $HOME
...
e.g., build on \AIXTM/\PowerPCTM\ for a
...
\item {\tt JIKES} the \JikesTM{} compiler executable ({\tt jikes}).
...
\item {\em various basic \UnixR{} utilities} e.g., {\tt grep}, {\tt xargs}, etc.
...
This section describes \JikesTM{} RVM's build configurations.
...
The \JikesTM{} RVM build process consists of two major phases:
the building of a 
{\em boot image}, and the building of a {\em boot loader}.
The boot image is built using a \JavaTM{} program executed within a host
...
For example, to build the prototype configuration for \AIXTM\
{\em on a Linux host}:
\begin{verbatim}
