...
0.11, nor with Classpath CVS head, as of this writing, 04 October
2004.  Steven Augart is working to remedy that deficiency and expects
to have it fixed soon. 
%% The cvs head of Jikes RVM normally also works with the
%% cvs head of GNU Classpath and with GNU Classpath 0.09 on Linux/IA32.
%% However, we are currently recommending that users do not upgrade to
%% Classpath 0.09.  There is a severe performance problem with Classpath
%% 0.09's I/O layer and Classpath 0.09 does not compile out-of-the-box on
%% AIX.  We hope that these issues will be resolved for Classpath 0.10.
...
You {\bf must rename} ``\texttt{\$\{RVM\_\-ROOT\}/{\it
    class\-path-0.10-build}/\-class\-path-0.10}'' to ``\texttt{\$\{RVM\_\-ROOT\}/\-{\it
    class\-path-0.10-build}/\-class\-path}''.  Now set your {\tt
  CLASS\-PATH\_\-ROOT} to ``\texttt{\$\{RVM\_\-ROOT\}/\-{\it class\-path-0.10-build}}''
({\bf not} to \texttt{\$\{RVM\_\-ROOT\}/\-{\it
    class\-path-0.10-build}/\-class\-path}).
