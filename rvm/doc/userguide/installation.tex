...
\item The Classpath jazzlib library, which implements java.util.zip
and java.util.jar 
\end{itemize}
...
\xlink{jazzlib}{\jazzlibjarURL} page.  However, you do not need to
explicitly download the 
second and third items; the Jikes RVM build process will automatically
check that these two items are present. If they are not present, the
files will be automatically downloaded, after asking for your
confirmation. 
...
Next, extract the Jikes RVM standard libraries, or ignore this step and
it will be performed automatically for you when you try to build a
...
If you downloaded the jazzlib library, place it also in 
{\tt \$RVM\_ROOT/support/lib}.  If you did not download the library,
