...
Use the pre-built rpm file for jikes-1.18 or higher.   (jikes-1.15 in
...
\item Setup a build directory with `jconfigure prototype', or
...
into some depth here.  For this example, we'll assume you downloaded {\tt classpath-0.05.tgz}.  Create a directory named something like {\tt
\$RVM\_ROOT/classpath-0.05-build}.  
A command like  {\tt /usr/gnu/tar -C \$\{RVM\_ROOT\}/classpath-0.05-build xzf classpath-0.05.tgz} will
create the directory {\tt \$\{RVM\_ROOT\}/classpath-0.05-build/classpath-0.05}.  You should
rename {\tt \$\{RVM\_ROOT\}/classpath-0.05-build/classpath-0.05} 
to {\tt \$\{RVM\_ROOT\}/classpath-0.05-build/classpath}.  Now set your
{\tt CLASSPATH\_ROOT} to {\tt \$\{RVM\_ROOT\}/classpath-0.05-build}.  
...
that will compile classes loaded at runtime using the optimizing
compiler and that uses a non-generational semi-space copying garbage
collector, use the name {\em BaseOptSemiSpace}.
