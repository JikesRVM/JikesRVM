 ...
 */
public final class ReferenceDensityComparator implements Comparator<BootImageMap.Entry> {
  private final Comparator<BootImageMap.Entry> identicalSizeComparator;
  public ReferenceDensityComparator(Comparator<BootImageMap.Entry> identicalSizeComparator) {
  ...
  public int compare(BootImageMap.Entry a, BootImageMap.Entry b) {
    ...
    if ((!aRef.isResolved() && !bRef.isResolved()) || (aRef == bRef)) {
      ...
      double aSize = (double)BootImageWriter.getNumberOfReferences(aRef.peekType(), a.getJdkObject()) / (double)BootImageWriter.getSize(aRef.peekType(), a.getJdkObject());
      double bSize = (double)BootImageWriter.getNumberOfReferences(bRef.peekType(), b.getJdkObject()) / (double)BootImageWriter.getSize(bRef.peekType(), b.getJdkObject());
