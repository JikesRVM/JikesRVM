...
public final class ReferenceDensityComparator implements Comparator<BootImageMap.Entry> {
  ...
  public int compare(BootImageMap.Entry a, BootImageMap.Entry b) {
    ...
    if ((!aRef.isResolved() && !bRef.isResolved()) || (aRef == bRef)) {
      ...
      double aSize = (double)getNumberOfReferences(aRef.peekType(), a.getJdkObject()) / (double)getSize(aRef.peekType(), a.getJdkObject());
      double bSize = (double)getNumberOfReferences(bRef.peekType(), b.getJdkObject()) / (double)getSize(bRef.peekType(), b.getJdkObject());
